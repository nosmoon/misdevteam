...
<%
	...
	rx.add(comboSet, "xx_dlco_clsf_cd", ds.xx_dlco_clsf_cd);
	...
	try {
		...
		for(int i = 0; i < ds.erplace_cd_curlist.size(); i++) {
			FC_ACCT_8070_MERPLACE_CD_CURLISTRecord rec = (FC_ACCT_8070_MERPLACE_CD_CURLISTRecord)ds.erplace_cd_curlist.get(i);
		...
		for(int i = 0; i < ds.xx_buff_3_curlist.size(); i++) {
			FC_ACCT_8070_MXX_BUFF_3_CURLISTRecord rec = (FC_ACCT_8070_MXX_BUFF_3_CURLISTRecord)ds.xx_buff_3_curlist.get(i);
			int record = rx.add(recordSet, "record", "");
