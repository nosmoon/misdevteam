...
<%
	...
	try {
		...
		for(int i = 0; i < ds.cur_slip_clsf_list.size(); i++) {
			FC_ACCT_2610_MCUR_SLIP_CLSF_LISTRecord rec = (FC_ACCT_2610_MCUR_SLIP_CLSF_LISTRecord)ds.cur_slip_clsf_list.get(i);
		...
		for(int i = 0; i < ds.cur_dlco_list.size(); i++) {
			FC_ACCT_2610_MCUR_DLCO_LISTRecord rec = (FC_ACCT_2610_MCUR_DLCO_LISTRecord)ds.cur_dlco_list.get(i);
		...
		for(int i = 0; i < ds.cur_clsf_list.size(); i++) {
			FC_ACCT_2610_MCUR_CLSF_LISTRecord rec = (FC_ACCT_2610_MCUR_CLSF_LISTRecord)ds.cur_clsf_list.get(i);
...
<% /* 작성시간 : Mon Nov 17 14:34:44 KST 2014 */ %>