...
<%
	RwXml rx = new RwXml();
	FC_FUNC_6540_MDataSet ds = (FC_FUNC_6540_MDataSet)request.getAttribute("ds");
	...
	try {
		...
		for(int i = 0; i < ds.curlist2.size(); i++) {
			FC_FUNC_6540_MCURLIST2Record rec = (FC_FUNC_6540_MCURLIST2Record)ds.curlist2.get(i);
		...
		for(int i = 0; i < ds.curlist1.size(); i++) {
			FC_FUNC_6540_MCURLIST1Record rec = (FC_FUNC_6540_MCURLIST1Record)ds.curlist1.get(i);
			int record = rx.add(recordSet, "record", "");
			rx.add(record, "cd", rec.cd);
			rx.add(record, "cdnm", rec.cdnm);
			rx.add(record, "cd_abrv_nm", rec.cd_abrv_nm);
		...
		for(int i = 0; i < ds.xx_budg_cd_list.size(); i++) {
			FC_FUNC_6540_MXX_BUDG_CD_LISTRecord rec = (FC_FUNC_6540_MXX_BUDG_CD_LISTRecord)ds.xx_budg_cd_list.get(i);
			int record = rx.add(recordSet, "record", "");
