<%@	page contentType="text/xml; charset=EUC-KR" %>

<%@	page import="
		java.io.*
	,	java.sql.*
	,	java.util.*
	,	kr.co.comsquare.rwXmlLib.*
	,	kr.co.comsquare.db.*
	,	kr.co.comsquare.util.Tokenizer
	,	chosun.ciis.co.base.util.*
	,	chosun.ciis.fc.func.rec.*
	,	chosun.ciis.fc.func.ds.*;
	"
%>

<%
	RwXml rx = new RwXml();
	FC_FUNC_1044_MDataSet ds = (FC_FUNC_1044_MDataSet)request.getAttribute("ds");
	int root = RwXml.rootNodeID;
	int dataSet = 0;
	int recordSet = 0;
	String errcode = ds.errcode;
	String errmsg = ds.errmsg;
	
	dataSet = rx.add(root, "fc_func_1044_m", "");
	rx.add(dataSet, "xx_now_note_stat_cd", ds.getXx_now_note_stat_cd());
	rx.add(dataSet, "xx_stat_cd", ds.getXx_stat_cd());
	rx.add(dataSet, "xx_dlco_clsf_cd", ds.getXx_dlco_clsf_cd());
	//System.out.println("데이터:"+ds.getXx_now_note_stat_cd());

	try {
		/****** XX_CD BEGIN */
		recordSet = rx.add(dataSet, "XX_CD", "");
		for(int i = 0; i < ds.xx_cd.size(); i++) {
			FC_FUNC_1044_MXX_CDRecord rec = (FC_FUNC_1044_MXX_CDRecord)ds.xx_cd.get(i);
			int record = rx.add(recordSet, "record", "");
			rx.add(record, "cd_nm", rec.cd_nm);
			rx.add(record, "cd", rec.cd);
			//rx.add(record, "cd_nm", StringUtil.replaceToXml(rec.cd_nm));
			//rx.add(record, "cd", StringUtil.replaceToXml(rec.cd));
		}
		rx.add(recordSet, "totalcnt", ds.xx_cd.size());
		/****** XX_CD END */
	}
	catch (Exception e) {
		errcode += " JSP Error";
		errmsg += " JSP Error Message = " + e.getMessage();
	}
	finally {
		rx.add(dataSet, "errcode", errcode);
		rx.add(dataSet, "errmsg", errmsg);
		out.println(rx.xmlFlush());
		out.println(rx.xmlEndFlush());
	}
%>

<% /* 작성시간 : Thu Dec 29 20:08:34 KST 2011 */ %>