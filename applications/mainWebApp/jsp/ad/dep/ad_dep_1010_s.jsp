...
<% 
	RwXml rx = new RwXml();
	AD_DEP_1010_SDataSet ds = (AD_DEP_1010_SDataSet)request.getAttribute("ds");
...
//대변
		...
		for(int i = 0; i < ds.curlist2.size(); i++) {
			AD_DEP_1010_SCURLIST2Record rec = (AD_DEP_1010_SCURLIST2Record)ds.curlist2.get(i);
...
//차변:어음내역			
		...
		for(int i=0; i<ds.curlist1.size(); i++){ 
			AD_DEP_1010_SCURLIST1Record rec = (AD_DEP_1010_SCURLIST1Record)ds.curlist1.get(i);
...
//차변:계좌입금	
		...
		for(int i = 0; i < list_02.size(); i++) {
			AD_DEP_1010_SCURLIST2Record rec = (AD_DEP_1010_SCURLIST2Record)list_02.get(i);
			...
			rx.addCData(record, "rmtt_plac", rec.rmtt_plac);  
			rx.add(record, "occr_dt", rec.occr_dt);
			rx.add(record, "slip_clsf", rec.slip_clsf);
			rx.add(record, "seq", rec.seq);			
			rx.add(record, "subseq", rec.subseq);
			...
			rx.add(record, "card_yn", rec.card_yn);
		...
		for(int i = 0; i <1-list_02.size(); i++) {
			...
			rx.add(record, "card_yn", "");
		}

//차변:선수금대체		
		int gridData3 = rx.add( resData , "gridData3" , "");
		for(int i = 0; i < list_04.size(); i++) {
			AD_DEP_1010_SCURLIST2Record rec = (AD_DEP_1010_SCURLIST2Record)list_04.get(i);
...
//차변:예수금대체		
		...
		for(int i = 0; i < list_06.size(); i++) {
			AD_DEP_1010_SCURLIST2Record rec = (AD_DEP_1010_SCURLIST2Record)list_06.get(i);

			long	amt	=	StringUtil.toLong(rec.amt);
...
//차변:기타입금대체		
		...
		for(int i = 0; i < list_07.size(); i++) {
			AD_DEP_1010_SCURLIST2Record rec = (AD_DEP_1010_SCURLIST2Record)list_07.get(i);
			...
			rx.add(record, "subseq", rec.subseq);	
			if("24".equals(ds.slip_clsf)){
				rx.add(record, "setoff_slip_no",rec.rcpm_slip_no);
			}else{
				rx.add(record, "setoff_slip_no","");
			}
			rcpm_tot_amt 	+= 	amt;	
			advt_tot_amt 	+= 	amt;		
			amt_07			+=	amt;
...
//대변:선수금
		...
		for(int i = 0; i < list_08.size(); i++) {
			AD_DEP_1010_SCURLIST2Record rec = (AD_DEP_1010_SCURLIST2Record)list_08.get(i);
			long	amt	=	StringUtil.toNumber(rec.amt);
...
//대변:예수금
		...
		for(int i = 0; i < list_09.size(); i++) {
			AD_DEP_1010_SCURLIST2Record rec = (AD_DEP_1010_SCURLIST2Record)list_09.get(i);
			long	amt	=	StringUtil.toLong(rec.amt);
...
//대변:기타입금
		...
		for(int i = 0; i < list_10.size(); i++) {
			AD_DEP_1010_SCURLIST2Record rec = (AD_DEP_1010_SCURLIST2Record)list_10.get(i);
			long	amt	=	StringUtil.toNumber(rec.amt);
		...
		rx.addCData(resForm, "rcpm_acct_nm", rcpm_acct_nm);
