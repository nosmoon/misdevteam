...
<%
	RwXml rx = new RwXml();
	AD_PUB_3013_LDataSet ds = (AD_PUB_3013_LDataSet)request.getAttribute("ds");
...
//	int resData = rx.add(root, "resData", "");
	...
	try {
		for(int i = 0; i < ds.curlist.size(); i++) {
			AD_PUB_3013_LCURLISTRecord rec = (AD_PUB_3013_LCURLISTRecord)ds.curlist.get(i);
			int record = 0;
			//기타 나오기 전까지의 데이터로 소계내기
			if("999".equals(rec.grp_cmpy_cd)){
				record = rx.add(gridData, "record", "");
				rx.add(record, "grp_cmpy_cd"		, "");
				rx.add(record, "grp_cmpy_nm"		, "그룹사 소계");
	
				rx.add(record, "advt_fee_1"		, tot_advt_fee_1);
				rx.add(record, "vat_1"			, "");
				rx.add(record, "pubc_tot_amt_1"	, "");
				rx.add(record, "cnt_1"			, tot_cnt_1);
				
				rx.add(record, "advt_fee_2"		, tot_advt_fee_2);
				rx.add(record, "vat_2"			, "");
				rx.add(record, "pubc_tot_amt_2"	, "");
				rx.add(record, "cnt_2"			, tot_cnt_2);
				
				rx.add(record, "advt_fee_3"		, tot_advt_fee_3);
				rx.add(record, "vat_3"			, "");
				rx.add(record, "pubc_tot_amt_3"	, "");
				rx.add(record, "cnt_3"			, tot_cnt_3);
				//전년비교
				long so_hap_1 = tot_advt_fee_1 - tot_advt_fee_2;
				double so_ratio_1 =  ((double)so_hap_1/(double)tot_advt_fee_2) * 100.0;
				
				//전전년비교
				long so_hap_2 = tot_advt_fee_1 - tot_advt_fee_3;
				double so_ratio_2 =  ((double)so_hap_2/(double)tot_advt_fee_3) * 100.0;
				
				rx.add(record, "diff_1"			, so_hap_1);
				rx.add(record, "diff_2"			, so_hap_2);
				rx.add(record, "diff_ratio_1"	, so_ratio_1);
				rx.add(record, "diff_ratio_2"	, so_ratio_2);
				rx.add(record, "max_pubc_dt"	, "");			
			}
			record = rx.add(gridData, "record", "");
