...
<%
    ...
    RwXml rx = new RwXml();
	AD_PUB_1851_LDataSet ds = (AD_PUB_1851_LDataSet)request.getAttribute("ds");
	...
	try {
		...
		long tot_pubc           = 0;//광고지사수수료합계로 사용함
				
		for(int i = 0; i < ds.curlist.size(); i++) {
			AD_PUB_1851_LCURLISTRecord rec = (AD_PUB_1851_LCURLISTRecord)ds.curlist.get(i);
			int record = rx.add(gridData, "record", "");
			rx.add(record,"sale_stmt_issu_yn",rec.sale_stmt_issu_yn);	//발행
			...
			rx.addCData(record,"dtls_medi_nm",rec.dtls_medi_nm);				//상세매체
			...
			rx.add(record,"sect_nm_cd",rec.sect_nm);						//섹션명
			rx.add(record,"pubc_clsf_nm",rec.pubc_clsf_nm);				//구분
			rx.add(record,"rslt_dept_nm",rec.rslt_dept);				//실적부서명			
			rx.add(record,"edt_clsf",rec.edt_clsf);				//판구분
			rx.add(record,"indt_clsf",rec.indt_clsf);//업종
			rx.add(record,"indt_clsf_nm",rec.indt_clsf_nm);//업종
			...
			if("4".equals(hndl_clsf)){
				tot_pubc      += StringUtil.toLong(rec.coms);
			}else if("10".equals(hndl_clsf)){
				tot_fee      += StringUtil.toLong(rec.coms);			
			}
			else{
