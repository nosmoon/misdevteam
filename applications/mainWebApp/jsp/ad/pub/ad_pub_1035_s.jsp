...
//	int resData = rx.add(root, "resData", "");
	...
	try { 
		...
		rx.add(resForm, "brdg_advt_yn", StringUtil.nvl(ds.brdg_advt_yn, "N")); 
		rx.add(resForm, "seq_clsf", ds.seq_clsf);  //순번구분
		rx.add(resForm, "nwsp_type_yn", ds.nwsp_type_yn);  //협찬광고여부
		rx.add(resForm, "nwsp_side_type", ds.nwsp_side_type);  //지면형태

		rx.add(resForm, "dtls_medi_cd", ds.dtls_medi_cd);
		rx.add(resForm, "dtev_medi_cd", ds.dtls_medi_cd);
		rx.add(resForm, "dtob_medi_cd", ds.dtls_medi_cd);
		rx.add(resForm, "pre_issu_yn", StringUtil.nvl(ds.pre_issu_yn, "N")); 
		rx.add(resForm, "pre_day_issu", StringUtil.nvl(ds.pre_day_issu, "N")); 		
		rx.add(resForm, "asist_pers", ds.asist_pers);  //어시스트 사번		   
		rx.add(resForm, "asist_pers_nm", ds.asist_pers_nm);  //어시스트명
		System.out.println("asist_pers_nm::"+ds.asist_pers_nm);	
...
*/			
		...
		for(int i = 0; i < ds.curlist.size(); i++) {
			AD_PUB_1035_SCURLISTRecord rec = (AD_PUB_1035_SCURLISTRecord)ds.curlist.get(i);
		...
		for(int i = 0; i < ds.curlist2.size(); i++) {
			AD_PUB_1035_SCURLIST2Record rec = (AD_PUB_1035_SCURLIST2Record)ds.curlist2.get(i);
