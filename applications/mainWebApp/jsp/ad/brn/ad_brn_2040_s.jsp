...
<%
	...
	try {
		/****** CURLIST2 BEGIN */
		if(ds.page_id.equals("AD_BRN_2000")){		//일일보고
			formData = rx.add(root, "formData", "");
			for(int i = 0; i < ds.curlist1.size(); i++) {
				AD_BRN_2040_SCURLIST1Record rec = (AD_BRN_2040_SCURLIST1Record)ds.curlist1.get(i);
				rx.add(formData, "cont", rec.cont);//System.out.println("cont::"+rec.cont);
				rx.add(formData, "taem_chf_opn", rec.taem_chf_opn);
			}		
		}else if(ds.page_id.equals("AD_BRN_2001")){	//일일보고 등록
			resData = rx.add(root, "resData", "");
			resform = rx.add(resData, "resform", "");			
			
			for(int i = 0; i < ds.curlist2.size(); i++) {
				AD_BRN_2040_SCURLIST2Record rec = (AD_BRN_2040_SCURLIST2Record)ds.curlist2.get(i);	
				switch (i){
    				case  0 :
