...
<%
	...
	try{
		...
		for(int i = 0; i < ds.curlist.size(); i++) {
			AD_BAS_2920_LCURLISTRecord rec = (AD_BAS_2920_LCURLISTRecord)ds.curlist.get(i);
			int record = rx.add(gridData, "record", "");
			
			long pubc_amt 		= StringUtil.toLong(rec.pubc_amt);
			long prvmm_pubc_amt = StringUtil.toLong(rec.prvmm_pubc_amt);
			
			double prvmm_medi_pubc_rate = ((double)(pubc_amt-prvmm_pubc_amt)/(double)prvmm_pubc_amt) * 100;
			  
			long rcpm_amt 		= StringUtil.toLong(rec.rcpm_amt);
			long prvmm_rcpm_amt = StringUtil.toLong(rec.prvmm_rcpm_amt);
			    
			double prvmm_medi_rcpm_rate = ((double)(rcpm_amt-prvmm_rcpm_amt)/(double)prvmm_rcpm_amt) * 100;
			...
			rx.add(record, "prvmm_medi_pubc_rate",  prvmm_pubc_amt == 0 ? "0" : DigitUtil.cutUnderPoint(prvmm_medi_pubc_rate,2));
			rx.add(record, "rcpm_amt",              rec.rcpm_amt                );
			rx.add(record, "unrcp_amt",             rec.unrcp_amt               );
			rx.add(record, "prvmm_rcpm_amt",        rec.prvmm_rcpm_amt          );
			rx.add(record, "prvmm_medi_rcpm_rate",  prvmm_rcpm_amt == 0 ? "0" : DigitUtil.cutUnderPoint(prvmm_medi_rcpm_rate,2));
			rx.add(record, "medi_rcpm_icdc_rank",   rec.medi_rcpm_icdc_rank      );
			rx.add(record, "dhon_amt",              rec.dhon_amt                );
			rx.add(record, "medi_dhon_rank",        rec.medi_dhon_rank           );
			rx.add(record, "dhon_bal",              rec.dhon_bal                );
			rx.add(record, "medi_dhon_bal_rank",    rec.medi_dhon_bal_rank       );
