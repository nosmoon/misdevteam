...
<%
	...
	int resData    = rx.add(root, "resData1", "");
	try{
	
		int gridData1  = rx.add(resData, "gridData1", "");
		for(int i = 0; i < ds.curlist1.size(); i++) {
			AD_BAS_3110_LCURLIST1Record rec = (AD_BAS_3110_LCURLIST1Record)ds.curlist1.get(i);
			int record = rx.add(gridData1, "record", "");
			
			rx.add(record, "pubc_dt", rec.pubc_dt);
			rx.add(record, "wkdy", rec.wkdy);
			rx.add(record, "tot_side", rec.tot_side); 
			rx.add(record, "tot_dn", DigitUtil.cutUnderPoint(rec.tot_dn,1));
			rx.add(record, "clr_dn", DigitUtil.cutUnderPoint(rec.clr_dn,1));  
			rx.add(record, "advt_fee", rec.advt_fee);
			rx.add(record, "advt_cnt", rec.advt_cnt); 
			rx.add(record, "advt_ocpy_rate", DigitUtil.cutUnderPoint(rec.advt_ocpy_rate,1));
			rx.add(record, "spon_advt_fee", rec.spon_advt_fee);
			rx.add(record, "spon_advt_cnt", rec.spon_advt_cnt);
		}
		
	
		int gridData2  = rx.add(resData, "gridData2", "");
		for(int i = 0; i < ds.curlist2.size(); i++) {
			AD_BAS_3110_LCURLIST2Record rec = (AD_BAS_3110_LCURLIST2Record)ds.curlist2.get(i);
			...
			long avg_dn 			= StringUtil.toLong(rec.avg_dn);
			long avg_advt_ocpy_rate = StringUtil.toLong(rec.avg_advt_ocpy_rate);
			long tot_spon_advt_fee	= StringUtil.toLong(rec.tot_spon_advt_fee);
			...
			rx.add(record, "avg_dn", StringUtil.nvl(rec.avg_dn, "0"));
			rx.add(record, "avg_advt_ocpy_rate", DigitUtil.cutUnderPoint(rec.avg_advt_ocpy_rate, 1));
			rx.add(record, "tot_spon_advt_fee", StringUtil.nvl(rec.tot_spon_advt_fee, "0"));	
						
			
			if(i == ds.curlist2.size()-1){
				AD_BAS_3110_LCURLIST2Record pre_rec = (AD_BAS_3110_LCURLIST2Record)ds.curlist2.get(i-1);
				...
				long pre_avg_dn 			= StringUtil.toLong(pre_rec.avg_dn);
				long pre_avg_advt_ocpy_rate = StringUtil.toLong(pre_rec.avg_advt_ocpy_rate);
				long pre_tot_spon_advt_fee	= StringUtil.toLong(pre_rec.tot_spon_advt_fee);
				...
				rx.add(record, "avg_dn", 			pre_avg_dn-avg_dn);
				rx.add(record, "avg_advt_ocpy_rate",DigitUtil.cutUnderPoint(pre_avg_advt_ocpy_rate-avg_advt_ocpy_rate, 1));
				rx.add(record, "tot_spon_advt_fee", pre_tot_spon_advt_fee-tot_spon_advt_fee);
				...
				double r_avg_dn   				= avg_dn == 0 ? 100 : ((double)(pre_avg_dn-avg_dn)/(double)avg_dn) * 100;
				...
				double r_tot_spon_advt_fee   	= tot_spon_advt_fee == 0 ? 100 : ((double)(pre_tot_spon_advt_fee-tot_spon_advt_fee)/(double)tot_spon_advt_fee) * 100;
				...
				rx.add(record, "avg_dn", DigitUtil.cutUnderPoint(r_avg_dn,0));
				rx.add(record, "avg_advt_ocpy_rate", DigitUtil.cutUnderPoint(r_avg_advt_ocpy_rate,0));
				rx.add(record, "tot_spon_advt_fee", DigitUtil.cutUnderPoint(r_tot_spon_advt_fee,0));
