...
<%
	RwXml rx = new RwXml();
	CO_SMTB_1101_LDataSet ds = (CO_SMTB_1101_LDataSet)request.getAttribute("ds");
	...
	try {
		...
		for(int i = 0; i < ds.tax_stmtlist.size(); i++) {
			CO_SMTB_1101_LTAX_STMTLISTRecord rec = (CO_SMTB_1101_LTAX_STMTLISTRecord)ds.tax_stmtlist.get(i);
			...
			rx.addCData(record, "amend_code", rec.amend_code);
			rx.addCData(record, "amend_code_nm", rec.amend_code_nm);
			rx.addCData(record, "sup_email", rec.sup_email);
			rx.addCData(record, "cash_amount", rec.cash_amount);
			rx.addCData(record, "note_amount", rec.note_amount);
			rx.addCData(record, "check_amount", rec.check_amount);
			rx.addCData(record, "receivable_amount", rec.receivable_amount);
			...
			//rx.add(record, "amend_code", StringUtil.replaceToXml(rec.amend_code));
			//rx.add(record, "amend_code_nm", StringUtil.replaceToXml(rec.amend_code_nm));
			//rx.add(record, "sup_email", StringUtil.replaceToXml(rec.sup_email));
			//rx.add(record, "cash_amount", StringUtil.replaceToXml(rec.cash_amount));
			//rx.add(record, "note_amount", StringUtil.replaceToXml(rec.note_amount));
			//rx.add(record, "check_amount", StringUtil.replaceToXml(rec.check_amount));
			//rx.add(record, "receivable_amount", StringUtil.replaceToXml(rec.receivable_amount));
		}
