...
<%
	...
	try {
		...
		for(int i = 0; i < ds.curlist.size(); i++) {
			IS_BAS_1110_LCURLISTRecord rec = (IS_BAS_1110_LCURLISTRecord)ds.curlist.get(i);
		...
		for(int i = 0; i < ds.curlist.size(); i++) {
			...
			if(Integer.parseInt(rec.step) == 0){
				...
				for(int j = 0; j < ds.curlist.size(); j++) {
					IS_BAS_1110_LCURLISTRecord sub_rec = (IS_BAS_1110_LCURLISTRecord)ds.curlist.get(j);
					if(Integer.parseInt(sub_rec.step) == 1 && sub_rec.supr_dept_cd.equals(rec.dept_cd)){
						...
						for(int k = 0; k < ds.curlist.size(); k++) {
							IS_BAS_1110_LCURLISTRecord sub_1_rec = (IS_BAS_1110_LCURLISTRecord)ds.curlist.get(k);
							if(Integer.parseInt(sub_1_rec.step) == 2 && sub_1_rec.supr_dept_cd.equals(sub_rec.dept_cd)){
								...
								for(int l = 0; l < ds.curlist.size(); l++) {
									IS_BAS_1110_LCURLISTRecord sub_2_rec = (IS_BAS_1110_LCURLISTRecord)ds.curlist.get(l);
