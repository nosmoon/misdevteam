...
<%
	...
	try {
		...
		rx.add(resForm, "card_rcpm_reg_dt" 			,ds.card_rcpm_reg_dt);	
		rx.add(resForm, "card_rcpm_reg_seq" 		,ds.card_rcpm_reg_seq);	
		
		
		rx.add(resForm, "dlco_no" 			        ,ds.dlco_clsf_cd_seq);	
		rx.add(resForm, "dlco_nm" 					,ds.dlco_nm);
		rx.add(resForm, "rcpm_tot_amt" 				,ds.rcpm_tot_amt);
		rx.add(resForm, "pubc_tot_amt" 				,ds.rcpm_tot_amt);
		...
		rx.add(resForm, "proc_frdt" 				,ds.proc_frdt);
		rx.add(resForm, "proc_todt" 				,ds.proc_todt);	
		...
		for(int i = 0; i<ds.curlist.size(); i++) {
			IS_DEP_4051_LCURLISTRecord rec = (IS_DEP_4051_LCURLISTRecord)ds.curlist.get(i);
			int record = rx.add(gridData, "record", "");
			rx.add(record, "setoff_yn", "N");
			rx.add(record, "rcpm_suply_amt", rec.rcpm_suply_amt);    //상계금액              
			...
			rx.add(record, "unrcpm_amt", Integer.parseInt(rec.unrcpm_suply_amt) + Integer.parseInt(rec.unrcpm_vat_amt)); /**미수액**/
