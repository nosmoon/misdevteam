...
<%
	...
	rec_aplcno = request.getParameter("rec_aplcno");
...
<script language="javascript">
	...
	function resAlert(){
		var msg="<%=request.getAttribute("resMsg")%>";
		var aplcpath="<%=request.getParameter("aplcpath")%>";
		alert(msg);
		
		if(aplcpath=="SHFT"){
			parent.jsSubmit('<%=request.getParameter("accflag")%>');
		}else if(aplcpath="NWSP"){
			parent.jsInit();
			// 상세정보 폼의 값을 reset 해준다.
			parent.nwspsubscntr_list_search("<%=request.getParameter("rdr_no")%>", "<%=request.getParameter("medicd")%>", "s");  // "s" 상세조회후 call 한것임을 나타내는 flag
		}			
	}
</script>
