...
<%---------------------------------------------------------------------------------------------------
* 파일명 : ss_extn_1700_a.jsp
* 기능 : 확장현황-사이버확장-기준-초기화면
* 작성일자 : 2004-04-06
* 작성자 : 고윤홍
---------------------------------------------------------------------------------------------------%>
...
function camp_list_search(){
    ...
    // 기준목록 조회조건을 "/ss/extn/1705" 서블릿으로 보낸다
    lo_form1.action = "/ss/extn/1705";
...
function camp_detail_view(rdr_extncampno){
    ...
    // 사이버확장기준번호를 "/ss/extn/1710" 서블릿으로 보낸다
    lo_form1.rdr_extncampno.value = rdr_extncampno;
    lo_form1.action = "/ss/extn/1710";
    lo_form1.target = "ifrm";
    lo_form1.submit();
}
...
function camp_add()
{
	...
	var lo_table2 = document.all.camp_detail_id;		        	// 상세정보 기준정보 테이블
	lo_table1.rows[0].cells[0].innerText = '사이버확장기준 등록';
...
*/    
    ...
    for( var i = 1; i<= li_count; i++) {
    	...
    	if(document.all("medicd"+i).value == null || document.all("medicd"+i).value == ""){
    		// 매체가 존재하지 않은 경우 수당,권유자수당이 존재하면 경고
    		if(document.all("alonamt"+i).value != ''){
    			alert("매체를 선택한 후 수당을 입력하세요");
    			setEventHandler();
    			return;
    		}
    		if(document.all("cnvsalonamt"+i).value != ''){
    			alert("매체를 선택한 후 권유자수당을 입력하세요");
    			setEventHandler();
    			return;
    		}
    		continue;
    	}
    	// 매체가 존재할 경우, 수당, 권유자수당 여부 체크
    	if(document.all("medicd"+i).value != null && document.all("medicd"+i).value != ""){
    		...
    		if(document.all("cnvsalonamt"+i).value == null || document.all("cnvsalonamt"+i).value == ""){
    			alert("선택된 매체의 권유자수당을 입력하세요");
    			setEventHandler();
    			return;
    		}
    	}
        ...
        if(document.all("cnvsalonamt"+i).value != null && document.all("cnvsalonamt"+i).value != ""){
            ls_cnvsalonamtary = ls_cnvsalonamtary + document.all("cnvsalonamt"+i).value + "<%= paramGbn%>";
        }
    }
	...
	lo_form1.cnvsalonamtary.value = ls_cnvsalonamtary.substring(0, ls_cnvsalonamtary.length-2);

    if(lo_form1.accflag.value == 'I') {	        					// 신규입력일 때
        lo_form1.action = "/ss/extn/1715";
    } else if(lo_form1.accflag.value == 'U') {						// 수정저장일 때
        lo_form1.action = "/ss/extn/1720";
    } else {
      	return;
    }
...
function camp_del(){
    ...
    if(lo_form1.rdr_extncampno.value == '') {						// 확장기준번호 존재여부를 확인한다.
...
// 매체코드 선택시 관련 입력항목 제어
function medicd_change(medicd, alonamt, cnvsalonamt){
...
function page_move(pageno) {
    ...
    // 확장기준목록 조회조건을 "/ss/extn/1705" 서블릿으로 보낸다
    lo_form1.action = "/ss/extn/1705";
    lo_form1.target = "ifrm";
    lo_form1.submit();
}
...
<td width="340" valign="top">
    ...
    <table id="camp_detail_title_id" width="100%" border="0" cellspacing="0" cellpadding="2">
        <tr>
            <td height="25" bgcolor="#EBE9DC" class="title">사이버확장기준 등록</td>
        </tr>
        <tr>
            <td height="3">
            <!--여백-->
    ...
    <form name="camp_detail_form" method="post" action="">
        ...
        <input type="hidden" name="rdr_extncampno" value="">    <!-- 사이버확장기준번호 -->
        <input type="hidden" name="closyn" value="">            <!-- 사이버확장기준 마감여부 -->
        ...
        <tr>
            <td bgcolor="#dddddd"  class="gray" height="25">기준명<font color="#FF0000">*</td>
            <td colspan="2" class="gray" height="25">
                <input name="campnm" type="text" size="20" class="text_box" value="" caption="기준명" notEmpty>
            </td>
        </tr>
        <tr>
...
<%--   
        ...
        <tr>
            ...
            <td colspan="2" class="gray" height="25"> N </td>
        </tr>
        <tr>
            <td bgcolor="#dddddd" class="gray" height="25">지급일자<font color="#FF0000">*</td>
            <td colspan="2" class="gray" height="25">
                <input name="alonpayplandt" type="text" size="10" class="text_box" value="" caption="지급일자" dataType="date" notEmpty>
            </td>
        </tr>
--%>        
        <tr>
...
%> 
            ...
            <td class="gray" height="25" align="center">
                <input name="cnvsalonamt<%= i+1 %>" id="cnvsalonamt<%= i+1 %>" type="text" style="text-align:right" value="" size="8" class="text_box" caption="권유자수당" dataType="integer" comma>
            </td>
        </tr>
    <%}%>
    <%-- 매체 목록 반복 끝 --%>
