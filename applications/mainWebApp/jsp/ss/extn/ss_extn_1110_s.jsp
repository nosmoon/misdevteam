...
<%---------------------------------------------------------------------------------------------------
* 파일명 : ss_extn_1110_s.jsp
* 기능 : 확장현황-품앗이확장-대회관리-상세
* 작성일자 : 2003-11-25
* 작성자 : 김대섭
---------------------------------------------------------------------------------------------------%>
...
<table id="camp_detail_title_id" width="100%" border="0" cellspacing="0" cellpadding="2">
    <tr>
        <td height="25" bgcolor="#EBE9DC" class="title">품앗이확장대회 상세</td>
    </tr>
    <tr>
        <td height="3">
        <!--여백-->
...
<form name="camp_detail_form" method="post" action="">
    ...
    <input type="hidden" name="rdr_extncampno" value="<%= ds.getRdr_extncampno()%>"><!-- 품앗이확장대회번호 -->
    <input type="hidden" name="closyn" value="<%= ds.getClosyn()%>"><!-- 품앗이확장대회 마감여부 -->
    ...
    %>
        ...
        <%
            for(int j=0; j<ds.curmedicd.size(); j++){
                SS_S_RDR_EXTN_EXGCAMPCURMEDICDRecord curmedicdRec = (SS_S_RDR_EXTN_EXGCAMPCURMEDICDRecord)ds.curmedicd.get(j);
