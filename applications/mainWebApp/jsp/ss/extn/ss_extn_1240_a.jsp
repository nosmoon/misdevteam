...
<%---------------------------------------------------------------------------------------------------
* 파일명 : ss_extn_1240_a.jsp
* 기능 : 확장현황-사원확장-대회관리-비용-초기화면
* 작성일자 : 2003-12-12
* 작성자 : 김대섭
---------------------------------------------------------------------------------------------------%>
...
function jsInit(){
	...
	document.cost_search_form.frdt.focus();
}
...
// 대회 상세보기
function cost_detail_view(basidt){

    var lo_form1 = document.cost_list_form;
    // 사원확장대회번호를 "/ss/extn/1250" 서블릿으로 보낸다
    lo_form1.basidt.value = basidt;
    lo_form1.action = "/ss/extn/1250";
    lo_form1.target = "ifrm";
    lo_form1.submit();
}
...
function cost_add()
{
    var lo_form1 = document.cost_detail_form;	                    // 상세정보 폼
	var lo_table1 = document.all.cost_detail_title_id;		        // 상세정보 타이틀 테이블
	var lo_table2 = document.all.cost_detail_id;		        	// 상세정보 대회정보 테이블

	// 화면제어
	lo_table1.rows[0].cells[0].innerText = '사원확장비용마감 등록';	// 상세화면 제목 변경
...
function cost_save(){
    ...
    if(lo_form1.accflag.value == 'I') {	        					// 신규입력일 때
        lo_form1.action = "/ss/extn/1255";
    } else if(lo_form1.accflag.value == 'U') {						// 수정저장일 때
        lo_form1.action = "/ss/extn/1260";
    } else {
      return;
    }
    lo_form1.target = "ifrm";
...
//	lo_table1.rows[0].cells[0].innerText = '사원확장비용마감 상세';	// 상세화면 제목 변경
...
function cost_close(){
    ...
    lo_form1.target = "ifrm";
    lo_form1.action = "/ss/extn/1270";
    lo_form1.submit();
}

// 마감 취소
function cost_colsecancel(){
    var lo_form1 = document.cost_detail_form;
...
<tr>
    ...
    <form name="cost_search_form" method="post" action="" >
        ...
        <tr>
        	...
        	<td height="25" bgcolor="#EBE9DC"  class="title" align="right">
	            <input type="image" id="search_id" src="/html/comm_img/bu_search03.gif" width="36" height="19" align="absmiddle" onclick="javascript:cost_list_search();window.event.returnValue=false;">
        	</td>
        </tr>
        <tr>
            <td height="3">
    ...
    <!-- 사원확장비용 목록 시작 --------------------------------------------------------------------------------------------------->
    <table width="490"  cellpadding="1" cellspacing="0" class="gray" bgcolor="#ffffff" id="cost_list_id">
...
<td width="340" valign="top">
    ...
    <table id="cost_detail_title_id" width="100%" border="0" cellspacing="0" cellpadding="2">
        <tr>
            <td height="25" bgcolor="#EBE9DC" class="title">사원확장비용마감 등록</td>
        </tr>
        <tr>
            <td height="3">
            <!--여백-->
    ...
    <table id="cost_detail_id" width="340" border="0" cellspacing="0" cellpadding="2" bgcolor="#ffffff" class="gray">
    <form name="cost_detail_form" method="post" action="">
        <input type="hidden" name="accflag" value="I">          <!-- 입력(I),수정(U),삭제(D) 구분필드 초기값(I) -->
        <input type="hidden" name="closyn" value="">            <!-- 사원확장비용 마감여부 -->
        <tr>
            <td bgcolor="#dddddd"  width="90" class="gray" height="25">기준일자(년월)<font color="#FF0000">*</font></td>
            <td colspan="3" class="gray" height="25">
                <input name="basidt" type="text" size="7" class="text_box" value="" caption="기준일자" dataType="month" notEmpty>
            </td>
        </tr>
        <tr>
    ...
    <!---button-->
    <table width="340" border="0" cellspacing="0" cellpadding="0" class="title2" id="cost_button_id">
        <tr>
            <td align="center" height="2">
            <!--여백-->
            </td>
        ...
        <tr>
            <td align="center" height="24" bgcolor="#E8E8E8">
                <input type="image" id="add_id" src="/html/comm_img/bu_new.gif" width="39" height="19" border="0" align="absmiddle" onclick="javascript:cost_add();">
                <input type="image" id="save_id" src="/html/comm_img/bu_save.gif" width="39" height="19" border="0" align="absmiddle" onclick="javascript:cost_save();">
                <input type="image" id="del_id" src="/html/comm_img/bu_del.gif" width="39" height="19" border="0" align="absmiddle" onclick="javascript:cost_del();">
                <!-- 취소 버튼 -->
                <input type="image" id="cancel_id" src="/html/comm_img/bu_cancel.gif" width="39" height="19" align="absmiddle" onclick="javascript:cost_cancel();">
                <input type="image" id="close_id" src="/html/comm_img/bu_ma.gif" width="39" height="19" align="absmiddle" border="0" onclick="javascript:cost_close();">
                <input type="image" id="closecancel_id" src="/html/comm_img/bu_cancel01.gif" width="64" height="19" align="absmiddle" onclick="javascript:cost_colsecancel();">
            </td>
        </tr>
    </table>
    <!-- 사원확장비용 등록/상세 끝 --------------------------------------------------------------------------------------------------->
