...
<%---------------------------------------------------------------------------------------------------
* 파일명 : ss_extn_1280_a.jsp
* 기능 : 확장현황-사원확장-비용관리(담당)-초기화면
* 작성일자 : 2003-12-13
* 작성자 : 김대섭
---------------------------------------------------------------------------------------------------%>
...
*/
    ...
    // 확장신청목록 조회조건을 "/ss/extn/1282" 서블릿으로 보낸다
    lo_form1.action = "/ss/extn/1282";
    lo_form1.target = "ifrm";
    lo_form1.submit();
}
...
function check_all_chk(bool){
    ...
    if(lo_form1.busnchrgclamtyn==null) {						// 선택된 건이 없는 경우
        return;
    } else if(lo_form1.busnchrgclamtyn.type=="checkbox") {		// 한건인 경우
        lo_form1.busnchrgclamtyn.checked = bool;
    } else {													// 여러건인 경우
        for(var i=0; i<lo_form1.busnchrgclamtyn.length; i++) {
            lo_form1.busnchrgclamtyn[i].checked = bool;
        }
    }
}
...
function cost_detail_view(basidt, bocd){
    ...
    // 사원확장비용 기준일자, 지국코드를 "/ss/extn/1284" 서블릿으로 보낸다
    lo_form1.basidt.value = basidt;
    lo_form1.bocd.value = bocd;
    lo_form1.action = "/ss/extn/1284";
    lo_form1.target = "ifrm";
    lo_form1.submit();
}
...
function cost_save(){
    var lo_form1 = document.cost_list_form;
	if(lo_form1.busnchrgclamtyn == null){					// 체크박스가 없을 때
		return;
	}else if(lo_form1.busnchrgclamtyn.type == "checkbox"){	// 체크박스가 한개 일때
        lo_form1.basidtary.value = lo_form1.busnchrgclamtyn.value.substring(0, 6);
        lo_form1.bocdary.value = lo_form1.busnchrgclamtyn.value.substring(6, 11);
        if(lo_form1.busnchrgclamtyn.checked){
	    ...
	    for(var i=0; i<lo_form1.busnchrgclamtyn.length; i++) {
			...
			ls_temp = lo_form1.busnchrgclamtyn[i].value;
	        ...
	        if(lo_form1.busnchrgclamtyn[i].checked){
	...
	// 영업담당 처리 구분
	lo_form1.accflag.value = 'B';
    // 기준일자, 지국지사코드, 선택여부를 문자열 집합과 영업담당 처리구분 정보를 "/ss/extn/1286" 서블릿으로 보낸다.
    lo_form1.action = "/ss/extn/1286";
    lo_form1.target = "ifrm";
    lo_form1.submit();
}
...
%>                    
          ...
          <td class="title"  width="60">
            <select name="busnchrgclamtyn" size="1"class="sel_all">
              <option value="" selected>전체</option>
  ...
  <tr>
    <td height="10">
      ...
      <table width="830" border="0"  cellpadding="1" cellspacing="0">
        <tr>
          <td  height="27" bgcolor="#EBE9DC" class="title">사원확장대회 비용목록</td>
    ...
    <form name="cost_list_form" method="post" action="" >
		...
		<tr height="23" bgcolor="#dddddd" align="center">
