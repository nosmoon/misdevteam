...
<%
   SS_L_RDR_EXTNCLOS_INITDataSet ds = (SS_L_RDR_EXTNCLOS_INITDataSet)request.getAttribute("ds");
	...
	if(Integer.parseInt((Util.getDate()).substring(6))>20) {
		rxw.add(codeData, "closmm", (Util.getDate()).substring(0,6)); // 마감월
	} else {
		rxw.add(codeData, "closmm", Util.addMonth(Util.getDate(),-1).substring(0,6)); // 마감월
	}
...
rxw.add(codeData, "basicdt", "200907"); // 기간 from 
	
	rxw.add(codeData, "svy_stddt", "20151101"); // 기간 from 
	rxw.add(codeData, "svy_enddt", "20151231"); // 기간 from 
