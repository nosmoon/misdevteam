...
<%---------------------------------------------------------------------------------------------------
* 파일명 : ss_brsup_1200_a.jsp
* 기능 : 지국지원-신문지원물품-마감-초기화면
* 작성일자 : 2004-02-05
* 작성자 : 김대섭
---------------------------------------------------------------------------------------------------%>
...
function jsInit(){
	...
	document.nwspitemcostclos_search_form.frdt.focus();
}
...
// 목록보기
function nwspitemcostclos_list_search(){
    var lo_form1 = document.nwspitemcostclos_search_form;

    if(!check_all(lo_form1)) return;								// 폼 유효성 체크

    // 화면페이지 번호를 초기화한다.
    lo_form1.pageno.value = '1';
    // 마감목록 조회조건을 "/ss/brsup/1205" 서블릿으로 보낸다
    lo_form1.action = "/ss/brsup/1205";
    lo_form1.target = "ifrm";
    lo_form1.submit();
}

// 상세보기
function nwspitemcostclos_detail_view(basidt){

    var lo_form1 = document.nwspitemcostclos_list_form;
    // 기준일자를 "/ss/brsup/1210" 서블릿으로 보낸다
    lo_form1.basidt.value = basidt;
    lo_form1.action = "/ss/brsup/1210";
    lo_form1.target = "ifrm";
    lo_form1.submit();
}

// 추가
function nwspitemcostclos_add()
{
    var lo_form1 = document.nwspitemcostclos_detail_form;	        // 상세정보 폼
	var lo_table1 = document.all.nwspitemcostclos_detail_title_id; 	// 상세정보 타이틀 테이블
	var lo_table2 = document.all.nwspitemcostclos_detail_id;		// 상세정보 대회정보 테이블

	// 화면제어
	lo_table1.rows[0].cells[0].innerText = '신문지원물품마감 등록';	// 상세화면 제목 변경
...
// 저장
function nwspitemcostclos_save(){
   	var lo_form1 = document.nwspitemcostclos_detail_form;			// 상세정보 폼
    ...
    if(lo_form1.accflag.value == 'I') {	        					// 신규입력일 때
        lo_form1.action = "/ss/brsup/1215";
    } else if(lo_form1.accflag.value == 'U') {						// 수정저장일 때
        lo_form1.action = "/ss/brsup/1220";
    } else {
      return;
    }
    lo_form1.target = "ifrm";
...
function nwspitemcostclos_del(){
	...
	if(yesno){
	    ...
	    lo_form1.target = "ifrm";
	    lo_form1.action = "/ss/brsup/1225";
	    lo_form1.submit();
	}	    
}

// 취소
function nwspitemcostclos_cancel(){

    var lo_form1 = document.nwspitemcostclos_detail_form;	       	// 상세정보 폼
	var lo_table1 = document.all.nwspitemcostclos_detail_title_id;	// 상세정보 타이틀 테이블
	var lo_table2 = document.all.nwspitemcostclos_detail_id;		// 상세정보 대회정보 테이블

	lo_table1.rows[0].cells[0].innerText = '신문지원물품마감 상세';	// 상세화면 제목 변경
...
// 마감
function nwspitemcostclos_close(){
    var lo_form1 = document.nwspitemcostclos_detail_form;			// 상세정보 폼
    if(!check_all(lo_form1)) return;								// 폼 유효성 체크
    if(lo_form1.basidt.value == '') {								// 확장대회번호 존재여부를 확인한다.
    ...
    lo_form1.target = "ifrm";
    lo_form1.action = "/ss/brsup/1230";
    lo_form1.submit();
}

// 마감 취소
function nwspitemcostclos_colsecancel(){
    var lo_form1 = document.nwspitemcostclos_detail_form;
...
<tr>
    ...
    <form name="nwspitemcostclos_search_form" method="post" action="" >
        <input name="pageno" type="hidden" value ="1">
        <input name="pagesize" type="hidden" value ="<%= nwspitemcostclosListCount %>">
        ...
        <tr>
	        ...
	        <td height="25" bgcolor="#EBE9DC"  class="title" align="right">
	            <input type="image" id="search_id" src="/html/comm_img/bu_search03.gif" width="36" height="19" align="absmiddle" onclick="javascript:nwspitemcostclos_list_search();window.event.returnValue=false;">
        	</td>
        </tr>
        <tr>
            <td height="3">
    ...
    <form name="nwspitemcostclos_list_form" method="post" action="">
        ...
        <%-- 초기화면 목록 반복 시작 --%>
        <%for(int i=0; i<nwspitemcostclosListCount; i++){%>
        <tr height="25">
...
<td width="340" valign="top">
    ...
    <table id="nwspitemcostclos_detail_title_id" width="100%" border="0" cellspacing="0" cellpadding="2">
        <tr>
            <td height="25" bgcolor="#EBE9DC" class="title">신문지원물품마감 등록</td>
        </tr>
        <tr>
            <td height="3">
            <!--여백-->
    ...
    <form name="nwspitemcostclos_detail_form" method="post" action="">
        ...
        <input type="hidden" name="closyn" value="">            <!-- 신문지원물품마감 마감여부 -->
        <tr>
        ...
        <tr>
            ...
            <td colspan="3" class="gray" height="25">
                <input name="costclamdt" type="text" size="10" class="text_box" value="" caption="비용청구일자" dataType="date" notEmpty>
            </td>
        </tr>
    </form>
    </table>
    ...
    <!---button-->
    <table width="340" border="0" cellspacing="0" cellpadding="0" class="title2" id="nwspitemcostclos_button_id">
        <tr>
            <td align="center" height="2">
            <!--여백-->
            </td>
        ...
        <tr>
            <td align="center" height="24" bgcolor="#E8E8E8">
                <input type="image" id="add_id" src="/html/comm_img/bu_new.gif" width="39" height="19" border="0" align="absmiddle" onclick="javascript:nwspitemcostclos_add();">
                <input type="image" id="save_id" src="/html/comm_img/bu_save.gif" width="39" height="19" border="0" align="absmiddle" onclick="javascript:nwspitemcostclos_save();">
                <input type="image" id="del_id" src="/html/comm_img/bu_del.gif" width="39" height="19" border="0" align="absmiddle" onclick="javascript:nwspitemcostclos_del();">
                <!-- 취소 버튼 -->
                <input type="image" id="cancel_id" src="/html/comm_img/bu_cancel.gif" width="39" height="19" align="absmiddle" onclick="javascript:nwspitemcostclos_cancel();">
                <input type="image" id="close_id" src="/html/comm_img/bu_ma.gif" width="39" height="19" align="absmiddle" border="0" onclick="javascript:nwspitemcostclos_close();">
                <input type="image" id="closecancel_id" src="/html/comm_img/bu_cancel01.gif" width="64" height="19" align="absmiddle" onclick="javascript:nwspitemcostclos_colsecancel();">
            </td>
        </tr>
    </table>
    <!-- 신문지원물품마감 등록/상세 끝 --------------------------------------------------------------------------------------------------->
