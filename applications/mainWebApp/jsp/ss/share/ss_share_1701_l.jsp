...
<%---------------------------------------------------------------------------------------------------
* 파일명 : ss_share_1701_l.jsp
* 기능 : 정보공유 게시판 기사평 검색리스트 가져오기
* 작성일자 : 2003-12-11
* 작성자 : 고윤홍
---------------------------------------------------------------------------------------------------%>
...
function annc_write(){
    ...
    // 액션을 "/ss/share/1710" 서블릿으로 보낸다
    lo_form1.target = "_self";
    lo_form1.action = "/ss/share/1710";
    lo_form1.submit();
...
function page_move(pageno) {
    ...
    // 부서(팀) 또는 검색조건을 "/ss/share/1701" 서블릿으로 보낸다
...
function list_search(){
    ...
    // 부서(팀) 도는 검색조건을 "/ss/share/1701" 서블릿으로 보낸다
    lo_form1.target = "_self";
    lo_form1.action = "/ss/share/1701";
    lo_form1.submit();
...
function infoexg_view(seq,brwscnt) {
    ...
    // 시퀀스값을 "/ss/share/1720" 서블릿으로 보낸다
    lo_form1.seq.value = seq;
    lo_form1.brwscnt.value = brwscnt;
    lo_form1.target = "_self";
    lo_form1.action = "/ss/share/1720";
    lo_form1.submit();
}
...
// 지역 검색
function rmsgarea_search(){
    var lo_form1 = document.infoexg_form;
    if(lo_form1.rmsgdept.value == '') {
        var str = "<select id=rmsgarea_id name=\"rmsgarea\" class=\"sel_all\">" +
                      "<option value='' selected>전체</option>" +
                  "</select>"
        document.all.rmsgarea_id.outerHTML = str;
        return;
    }

    // 부서(팀)코드값을 "/ss/common/1103" 서블릿으로 보낸다
    lo_form1.common_deptcd.value = lo_form1.rmsgdept.value;
    lo_form1.action = "/ss/common/1103";
    lo_form1.target = "ifrm";
    lo_form1.submit();
}
-->
...
<table width="830" border="0" cellspacing="0" cellpadding="2" align="center">
  <tr>
	...
	<select name="rmsgdept" onChange="rmsgarea_search();" class="sel_all" <% if(!cmpycd.equals("100")) {%> disabled <%}%>>
<%
		writeDeptOpt(session, out, ds.buseolistOptHtml(rmsgdept), true);
%>
    </td>
    <td bgcolor="#EBE9DC" width="40" class="title">
    ...
    <td bgcolor="#EBE9DC" class="title" width="100">
        <select name="rmsgarea" size="1" id="rmsgarea_id" class="sel_all" <% if(!cmpycd.equals("100")) {%> disabled <%}%>>  <!--지역코드, 지역명 목록-->
        <%if("".equals(rmsgarea)){
        %>
            <option value='' selected>전체</option>
        <%}else{
        %>
        	<option value='<%= rmsgarea%>' selected><%= ds.getRmsgareanm_param()%></option>
        <%}%>
        </select>
    </td>
...
<table width="830" border="0"  cellpadding="1" cellspacing="0" align="center">
  <tr>
    <td  height="27" bgcolor="#EBE9DC" class="title">기사평 게시판 목록</td>
    <td  height="27" bgcolor="#EBE9DC" class="title" width="40" align="right">
    	&nbsp;<!--input type="image" id="insert_id" border="0" src="/html/comm_img/bu_write.gif" width="36" height="19" align="absmiddle" border="0" onclick="javascript:annc_write();window.event.returnValue=false""-->
    </td>
  </tr>
  <tr>
    <td height="3" colspan="2">
...
<table width="830" border="0" cellspacing="0" cellpadding="1" align="center" bgcolor="#FFFFFF" class="gray">
  <tr bgcolor="#dddddd" align="center">
    ...
    <td width="150" class="gray"> 첨부파일 </td>
  ...
  <tr>
    ...
    <td width="40" align="right" class="gray">&nbsp;<%= Util.comma(rec.brwscnt)%>&nbsp;</td>
    <td class="gray" >&nbsp;
