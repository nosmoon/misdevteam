...
<%-- dataset 인스턴스 선언부분--%>
<%
    SS_L_OUTRPT_INITDataSet ds = (SS_L_OUTRPT_INITDataSet)request.getAttribute("ds");    // request dataset
...
function jsInit(){
	...
	document.outrpt_form.makedt1.focus();
}
...
// 출장보고서 등록화면
function job_write(){
    var lo_form1 = document.outrpt_form;

    // 액션을 "/ss/share/1910" 서블릿으로 보낸다
    if(!check_all(lo_form1)) return;								// 폼 유효성 체크
    lo_form1.target = "_self";
    lo_form1.action = "/ss/share/1910";
    lo_form1.submit();
...
function list_search(){
    ...
    // 부서(팀) 또는 검색조건을 "/ss/share/1901" 서블릿으로 보낸다
    if(!check_all(lo_form1)) return;								// 폼 유효성 체크
    lo_form1.target = "_self";
...
// 출장보고서 상세
function outrpt_view(makedt,makepersid,brwscnt) {
    var lo_form1 = document.outrpt_form;

    // 시퀀스값을 "/ss/share/1920" 서블릿으로 보낸다
    if(!check_all(lo_form1)) return;								// 폼 유효성 체크
    ...
    lo_form1.target = "_self";
    lo_form1.action = "/ss/share/1920";
    lo_form1.submit();
}

//출장보고서 다운로드
function outrpt_dwload(makedt,makepersid,dwloadcnt) {
    var lo_form1 = document.outrpt_form;
...
<form name="outrpt_form" method="post" action="">
	<input type="hidden" name="makedt" value="">   <!--출장보고서 pk값-->
	<input type="hidden" name="makepersid" value=""> <!--출장보고서 pk값-->
...
<table width="830" border="0"  cellpadding="1" cellspacing="0" align="center">
  <tr>
    <td  height="27" bgcolor="#EBE9DC" class="title">출장보고서 목록</td>
...
<table width="830" border="0" cellspacing="0" cellpadding="1" align="center" bgcolor="#FFFFFF" class="gray">
  <tr bgcolor="#dddddd" align="center">
    ...
    for(int i=0 ; i< ds.outrptlist.size(); i++) {
        SS_L_OUTRPT_INITOUTRPTLISTRecord rec = (SS_L_OUTRPT_INITOUTRPTLISTRecord)ds.outrptlist.get(i);
    %>
  <tr>
    <td class="gray" width="250" align="center">
    	<a onClick="outrpt_view('<%=Util.checkString(rec.makedt)%>','<%= Util.checkString(rec.makepersid)%>','<%=rec.brwscnt%>');" style="cursor:hand;">
    ...
    <td class="gray" width="370" >&nbsp;
    	<a onClick="outrpt_dwload('<%=Util.checkString(rec.makedt)%>','<%= Util.checkString(rec.makepersid)%>','<%=rec.dwloadcnt%>');" style="cursor:hand;">
