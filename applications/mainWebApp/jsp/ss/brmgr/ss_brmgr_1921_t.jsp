...
//        String recpbankcd = request.getParameter("recpbankcd");
          String yymmdd = request.getParameter("sendyymmdd");

        String mediser_no_fr = request.getParameter("mediser_no_fr");
        String mediser_no_to = request.getParameter("mediser_no_to");

        response.reset();
        response.setHeader("Content-Type", "application/vnd.ms-excel; charset=KSC5601");
        response.setHeader("Content-Disposition", "attachment; filename=CRMWEEKBS_" + yymmdd + ".xls");
...
<%
    //  dataset 인스턴스 선언부분
    SS_P_SEND_PBLMEDISENDBODataSet ds = (SS_P_SEND_PBLMEDISENDBODataSet)request.getAttribute("ds");  // request dataset
