...
<!--
    ...
    function init(){
		...
		if((ls_rdr_extndt != "" && ls_rdr_extnno != "") || (ls_bocd != "" && ls_search_word != "" )){
            ...
            lo_form.action = "/ss/slcommon/1120"			;
            lo_form.target = "ifrm"							;
            lo_form.submit()								;
        }
    }
	...
	function reader_list_search(i_pageno){
        ...
        lo_form.bocd.value = "<%=Util.getSessionParameterValue(request, "bocd", true)%>";
        lo_form.pageno.value = i_pageno;
        lo_form.pagesize.value = "10";

		if(!check_all(lo_form, "/ss/slcommon/1120")) return;
		return;
	}
    ...
    function validate_reader_search_word(){
        ...
        var ls_word = lo_form.search_word.value;
       	...
       	if(ls_cond == "DSTC"){  		// 구역일 경우
           	if(!isValidFormat(ls_word, /^(\d{3})$/)){
            	alert("유효하지 않은 구역입니다.");
                lo_form.search_word.select();
                return false;
            }
       	}else if(ls_cond == "DSNO") {	// 구역+배달번호
        	if(!isValidFormat(ls_word, /^(\d{6})$/)){
        	...
        	if(!isValidFormat(ls_word, /^(\d{4,5}|\d{5,8}|\d{10,}|(\d{2,4})-{0,1}(\d{3,4})-{0,1}(\d{4}))$/)){
