...
<%
    ...
    //getting dataset
    SL_L_READER_01DataSet ds = (SL_L_READER_01DataSet)request.getAttribute("ds");  // request dataset

    //application exception 처리
    if(ds.readercur.size() < 1){
        request.setAttribute("appException", new AppException("sl_common_1120_l.jsp(1)","검색결과가 존재하지 않습니다."));
...
<%
		...
		for(int i=0; i<ds.readercur.size(); i++){
			rec = (SL_L_READER_01READERCURRecord)ds.readercur.get(i);
%>
