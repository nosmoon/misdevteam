...
<!--
    ...
    function init(){
		...
		if(ga_args[0] != ""){ //우편번호 검색시 우편번호를 argument 로 받았을 경우 검색을 실행한다.
            ...
            if(ga_args[1] == "A"){ // 특정 지국 코드를 입력하지 않으면 전체 우편번호내에서 검색
            	...
            	lo_form.bocd.value     		= "<%=Util.getSessionParameterValue(request, "bocd", true)%>"; //debugging
            }

			lo_form.search_word.value	= ga_args[0];
            lo_form.pageno.value     	= "1";
            lo_form.pagesize.value     	= "200";
            lo_form.action = "/ss/slcommon/1010";
            lo_form.target = "ifrm";
            lo_form.submit();
        }
    }
    ...
    function zip_search(i_pageno){
		...
		*/
        ...
        lo_form.pagesize.value     	= "100";

		if(!check_all(lo_form,"/ss/slcommon/1010")) return;
        return;
    }
