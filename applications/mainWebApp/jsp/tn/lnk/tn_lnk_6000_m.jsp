...
<%
	...
	try {
		...
		for(int i = 0; i < ds.supportlist.size(); i++) {
			TN_LNK_6000_MSUPPORTLISTRecord rec = (TN_LNK_6000_MSUPPORTLISTRecord)ds.supportlist.get(i);
		...
		for(int i = 0; i < ds.pymtmmlist.size(); i++) {
			TN_LNK_6000_MPYMTMMLISTRecord rec = (TN_LNK_6000_MPYMTMMLISTRecord)ds.pymtmmlist.get(i);
			int record = rx.add(recordSet, "record", "");
			rx.add(record, "cd", rec.cd);
			rx.add(record, "cdnm", rec.cdnm);
		}
		...
		for(int i = 0; i < ds.resultlist.size(); i++) {
			TN_LNK_6000_MRESULTLISTRecord rec = (TN_LNK_6000_MRESULTLISTRecord)ds.resultlist.get(i);
			int record = rx.add(recordSet, "record", "");
			rx.add(record, "cd", rec.cd);
			rx.add(record, "cdnm", rec.cdnm);
		}
