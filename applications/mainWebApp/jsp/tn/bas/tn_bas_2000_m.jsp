...
<%
	...
	try {
		...
		for(int i = 0; i < ds.xx_combo_020.size(); i++) {
			TN_BAS_2000_MXX_COMBO_020Record rec = (TN_BAS_2000_MXX_COMBO_020Record)ds.xx_combo_020.get(i);
		...
		for(int i = 0; i < ds.xx_combo_010.size(); i++) {
			TN_BAS_2000_MXX_COMBO_010Record rec = (TN_BAS_2000_MXX_COMBO_010Record)ds.xx_combo_010.get(i);
			int record = rx.add(recordSet, "item", "");
			rx.add(record, "cd", rec.cd);
			rx.add(record, "cd_nm", rec.cd_nm);
			//rx.add(record, "cd", StringUtil.replaceToXml(rec.cd));
...
<% /* 작성시간 : Wed Jun 22 18:16:46 KST 2016 */ %>