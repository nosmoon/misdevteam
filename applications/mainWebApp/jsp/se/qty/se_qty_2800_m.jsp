...
<%
	...
	rx.add(root		, "issu_yymm"	, today);
	
	formData				= rx.add(root		, "comboSet"		, "");
	try {
		recordSet			= rx.add(formData	, "teamlist"			, "");
		for(int i = 0; i < ds.teamlist.size(); i++) {
			SE_QTY_2800_MTEAMLISTRecord rec = (SE_QTY_2800_MTEAMLISTRecord)ds.teamlist.get(i);
		...
		for(int i = 0; i < ds.arealist.size(); i++) {
			SE_QTY_2800_MAREALISTRecord rec = (SE_QTY_2800_MAREALISTRecord)ds.arealist.get(i);
			int record = rx.add(recordSet, "record"			, "");
		...
		for(int i = 0; i < ds.partlist.size(); i++) {
			SE_QTY_2800_MPARTLISTRecord rec = (SE_QTY_2800_MPARTLISTRecord)ds.partlist.get(i);
			int record = rx.add(recordSet, "record"			, "");
			rx.add(record, "dept_cd"		, rec.dept_cd		);
			rx.add(record, "dept_nm"		, rec.dept_nm		);
			rx.add(record, "supr_dept_cd"	, rec.supr_dept_cd	);
		...
		for(int i = 0; i < ds.medilist.size(); i++) {
			SE_QTY_2800_MMEDILISTRecord rec = (SE_QTY_2800_MMEDILISTRecord)ds.medilist.get(i);
			int record = rx.add(recordSet, "record", "");
