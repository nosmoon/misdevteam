...
<%
	...
	try {
		...
		for(int i = 0; i < ds.team_cdcur.size(); i++) {
			SE_BOI_2400_MTEAM_CDCURRecord rec = (SE_BOI_2400_MTEAM_CDCURRecord)ds.team_cdcur.get(i);
		...
		for(int i = 0; i < ds.part_cdcur.size(); i++) {
			SE_BOI_2400_MPART_CDCURRecord rec = (SE_BOI_2400_MPART_CDCURRecord)ds.part_cdcur.get(i);
			int record = rx.add(recordSet, "record", "");
			rx.add(record, "dept_cd"		, rec.dept_cd);  		//부서코드
			rx.add(record, "dept_nm"		, rec.dept_nm);			//부서명
		...
		for(int i = 0; i < ds.area_cdcur.size(); i++) {
			SE_BOI_2400_MAREA_CDCURRecord rec = (SE_BOI_2400_MAREA_CDCURRecord)ds.area_cdcur.get(i);
			int record = rx.add(recordSet, "record", "");
