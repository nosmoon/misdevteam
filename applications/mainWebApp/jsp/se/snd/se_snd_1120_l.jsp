...
%>
<%---------------------------------------------------------------------------------------------------
* 파일명 : se_snd_1120_l.jsp
* 기능 : 판매 - 발송관리 - 노선등록 - 발송노선 정보 조회
* 작성일자 : 2009.02.03
* 작성자 : 김대준
---------------------------------------------------------------------------------------------------%>
...
<%
	...
	try {
		...
		StringBuffer title 		= new StringBuffer("발송처^지역^발송순서^탁송순위^합포여부^지국코드^발송처순번^지역코드^노선구분^노선코드^");
		...
		for ( int i = 0 ; i < len ; i++ ){
			...
			if ( "발송처".equals(titleVal) ){
				rt.addColumn("inputbutton", "ta:left;");
				size.append("80" + comma);
			} else if ( "발송순서" .equals(titleVal) || "탁송순위".equals(titleVal) ) {
				rt.addColumn("input", "format:#,###;ta:right;");			
				size.append("40" + comma);
			} else if ( "합포여부".equals(titleVal)) {
				rt.addColumn("checkbox", "checkvalue:1,2");
				size.append("40" + comma);
			} else if (   "지국코드".equals(titleVal) 
			           || "발송처순번".equals(titleVal) 
			           || "노선구분".equals(titleVal)
			           || "노선코드".equals(titleVal)
			           || "지역코드".equals(titleVal) ){
		...
		for ( int i = 0 ; i < ds.sendlist.size() ; i++ ){
			...
			for ( int j = 0 ; j < len ; j++ ){
				if ( j == 0 ){
					val = rec.send_plac_nm;							//발송처
				} else if ( j == 1 ){
					val = rec.area_nm;								//지역명
				} else if ( j == 2 ){
					val = rec.send_rank;							//발송순위
				} else if ( j == 3 ){
					val = rec.cnsg_seqo;							//탁송순서
				} else if ( j == 4 ){
					val = rec.send_mthd;							//합포여부
				} else if ( j == 5 ) {
					val = rec.bo_cd;								//지국코드
				} else if ( j == 6 ){
					val = rec.send_plac_seq;						//발송처순번
				} else if ( j == 7 ){
					val = rec.area_cd;								//지역코드
				} else if ( j == 8 ){
					val = rec.route_clsf;							//노선구분
				} else if ( j == 9 ){
					val = rec.route_cd;								//노선코드
				} else {
					if ( j - 10 < qtyArr.length ){
						val = qtyArr[j - 10];						//이외의 가변 필드 값
					} else {
						val = "";
					}
				}
