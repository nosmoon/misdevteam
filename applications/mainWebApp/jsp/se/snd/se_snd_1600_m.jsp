...
<%
	...
	try {
		...
		for(int i = 0; i < ds.prtlist.size(); i++) {
			SE_SND_1600_MPRTLISTRecord rec = (SE_SND_1600_MPRTLISTRecord)ds.prtlist.get(i);
		...
		for(int i = 0; i < ds.curlist.size(); i++) {
			SE_SND_1600_MCURLISTRecord rec = (SE_SND_1600_MCURLISTRecord)ds.curlist.get(i);
			int record = rx.add(recordSet, "record"	, "");
			rx.add(record, "cd"			, rec.cd		);
			rx.add(record, "cdnm"		,	 rec.cdnm	);
			rx.add(record, "cd_abrv_nm"	, rec.cd_abrv_nm);
		...
		for(int i = 0; i < ds.medilist.size(); i++) {
			SE_SND_1600_MMEDILISTRecord rec = (SE_SND_1600_MMEDILISTRecord)ds.medilist.get(i);
			int record = rx.add(recordSet, "record", "");
			rx.add(record, "cd"			, rec.cd		);
			rx.add(record, "cdnm"		, rec.cdnm		);
			rx.add(record, "cd_abrv_nm"	, rec.cd_abrv_nm);
...
<% /* 작성시간 : Thu Mar 12 17:39:24 KST 2009 */ %>