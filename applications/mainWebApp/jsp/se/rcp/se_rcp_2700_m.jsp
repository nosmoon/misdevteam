...
<%
	...
	try {
		rx.add(comboSet, "yymm", ds.yymm);  		//년월

		int teamlist = rx.add(comboSet, "teamlist", "");
		for(int i = 0; i < ds.team_cdcur.size(); i++) {
			SE_RCP_2700_MTEAM_CDCURRecord rec = (SE_RCP_2700_MTEAM_CDCURRecord)ds.team_cdcur.get(i);
		...
		for(int i = 0; i < ds.part_cdcur.size(); i++) {
			SE_RCP_2700_MPART_CDCURRecord rec = (SE_RCP_2700_MPART_CDCURRecord)ds.part_cdcur.get(i);
		...
		for(int i = 0; i < ds.area_cdcur.size(); i++) {
			SE_RCP_2700_MAREA_CDCURRecord rec = (SE_RCP_2700_MAREA_CDCURRecord)ds.area_cdcur.get(i);
