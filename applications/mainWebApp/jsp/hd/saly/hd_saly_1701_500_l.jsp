...
<%
	RwXml rx = new RwXml();
	HD_SALY_1701_500_LDataSet ds = (HD_SALY_1701_500_LDataSet)request.getAttribute("ds");
	...
	try {
		...
		for(int i = 0; i < ds.curlist.size(); i++) {
			HD_SALY_1701_500_LCURLISTRecord rec = (HD_SALY_1701_500_LCURLISTRecord)ds.curlist.get(i);
		...
		//	rx.add(record, "movm_fee", rec.movm_fee);
			...
			rx.add(record, "subj_sel_oli_deal_amt", rec.subj_sel_oli_deal_amt);
			rx.add(record, "make_proc_deal_amt", rec.make_proc_deal_amt);
		...
		//	rx.add(record, "gc_sply_edu_fee", rec.gc_sply_edu_fee);
			rx.add(record, "etc_alon_prize_amt", rec.etc_alon_prize_amt);
			rx.add(record, "etc_alon_show_amt", rec.etc_alon_show_amt);  
		    rx.add(record, "etc_bns_amt",           rec.etc_bns_amt);           
		    rx.add(record, "etc_evlu_amt",          rec.etc_evlu_amt);          
		    rx.add(record, "etc_etc_amt",           rec.etc_etc_amt);           
		    rx.add(record, "etc_spos_hlth_amt",     rec.etc_spos_hlth_amt);     
		    rx.add(record, "etc_ptph_amt",          rec.etc_ptph_amt);          
		    rx.add(record, "etc_med_exam_amt",      rec.etc_med_exam_amt);      
		    rx.add(record, "etc_child_birth_amt",   rec.etc_child_birth_amt);   
		    rx.add(record, "etc_scl_exps_amt",      rec.etc_scl_exps_amt);      
		    rx.add(record, "etc_drvr_trff_amt",     rec.etc_drvr_trff_amt);     
		    rx.add(record, "etc_cast_amt",          rec.etc_cast_amt);          
		    rx.add(record, "etc_edu_amt",           rec.etc_edu_amt);     
