...
<%
	...
	try {
		...
		rx.add(dataSet, "alvc_cnt", ds.getAlvc_cnt() );
		rx.add(dataSet, "aplc_vaca", ds.getAplc_vaca() );
		
		/****** CURLIST4 BEGIN */
		recordSet = rx.add(dataSet, "FAML_CLSF", "");

		for(int i = 0; i < ds.curlist4.size(); i++) {
			HD_VACA_1611_LCURLIST4Record rec = (HD_VACA_1611_LCURLIST4Record)ds.curlist4.get(i);
			int record = rx.add(recordSet, "item", "");
			rx.add(record, "faml_flnm", rec.faml_flnm);
			rx.add(record, "faml_birth", rec.faml_birth);
			//rx.add(record, "faml_flnm", StringUtil.replaceToXml(rec.faml_flnm));
			//rx.add(record, "faml_birth", StringUtil.replaceToXml(rec.faml_birth));
		}
		...
		for(int i = 0; i < ds.curlist3.size(); i++) {
			HD_VACA_1611_LCURLIST3Record rec = (HD_VACA_1611_LCURLIST3Record)ds.curlist3.get(i);
		...
		recordSet = rx.add(dataSet, "VACA_DTLS_44", ""); //출산전후휴가 2017.06.14 추가  : 제도 변경으로 인해 쌍둥이일 경우 휴가가 120일 주어짐 
		...
		recordSet = rx.add(dataSet, "VACA_DTLS_50", ""); //유산사산휴가  2017.06.16 추가  : 세부항목에서 선택가능하도록 수정  
		...
		recordSet = rx.add(dataSet, "VACA_DTLS_60", ""); //육아바우처  2018.03.09 추가  : 세부항목에서 선택가능하도록 수정  
		...
		for(int i = 0; i < ds.curlist2.size(); i++) {
			HD_VACA_1611_LCURLIST2Record rec = (HD_VACA_1611_LCURLIST2Record)ds.curlist2.get(i);
		...
		for(int i = 0; i < ds.curlist1.size(); i++) {
			HD_VACA_1611_LCURLIST1Record rec = (HD_VACA_1611_LCURLIST1Record)ds.curlist1.get(i);
			int record = rx.add(recordSet, "record", "");
		...
		for(int i = 0; i < ds.curlist.size(); i++) {
			HD_VACA_1611_LCURLISTRecord rec = (HD_VACA_1611_LCURLISTRecord)ds.curlist.get(i);
			int record = rx.add(recordSet, "record", "");
			rx.add(record, "emp_no", rec.emp_no);
			...
			//rx.add(record, "rotate_duty_yn", StringUtil.replaceToXml(rec.rotate_duty_yn));
		}
