	...
	try {
		...
		for(int i = 0; i < ds.curlist.size(); i++) {
			...
			if(Integer.parseInt(rec.lvl) == 0){
				...
				rx.add(record, "label", StringUtil.replaceToXml(rec.abrv_nm) + "(" + rec.dept_cd + ") ");
				...
				for(int j = 0; j < ds.curlist.size(); j++) {
					...
					if(Integer.parseInt(sub_rec.lvl) == 1 && sub_rec.insd_dept_cd.substring(0,4).equals(rec.insd_dept_cd)){
						...
						rx.add(sub_record, "label", StringUtil.replaceToXml(sub_rec.abrv_nm) + "(" + sub_rec.dept_cd + ") ");
						...
						for(int k = 0; k < ds.curlist.size(); k++) {
							HD_ORGA_1001_LCURLISTRecord sub_1_rec = (HD_ORGA_1001_LCURLISTRecord)ds.curlist.get(k);
							if(Integer.parseInt(sub_1_rec.lvl) == 2 && sub_1_rec.insd_dept_cd.substring(0,6).equals(sub_rec.insd_dept_cd)){
								int sub_1_record = rx.add(sub_record, "item", "");
									rx.add(sub_1_record, "label", StringUtil.replaceToXml(sub_1_rec.abrv_nm) + "(" + sub_1_rec.dept_cd + ") ");
								...
								for(int l = 0; l < ds.curlist.size(); l++) {
									HD_ORGA_1001_LCURLISTRecord sub_2_rec = (HD_ORGA_1001_LCURLISTRecord)ds.curlist.get(l);
									if(Integer.parseInt(sub_2_rec.lvl) == 3 && sub_2_rec.insd_dept_cd.substring(0,8).equals(sub_1_rec.insd_dept_cd)){
										...
										rx.add(sub_2_record, "label", StringUtil.replaceToXml(sub_2_rec.abrv_nm) + "(" + sub_2_rec.dept_cd + ") ");
					...
					//	int sub_1_record = rx.add(sub_record, "item", "");
					//	rx.add(sub_1_record, "label", StringUtil.replaceToXml(sub_rec.abrv_nm) + "(" + sub_rec.dept_cd + ") ");
...
<% /* 작성시간 : Wed Apr 22 15:08:06 KST 2009 */ %>