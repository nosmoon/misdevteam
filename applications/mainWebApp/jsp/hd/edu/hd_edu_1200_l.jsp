...
<%
	...
	try {
		...
		for(int i = 0; i < ds.curlist.size(); i++) {
			HD_EDU_1200_LCURLISTRecord rec = (HD_EDU_1200_LCURLISTRecord)ds.curlist.get(i);
			if ("TRAIN_FLAG".equals(rec.getCd_type())){
		...
		for(int i = 0; i < ds.curlist.size(); i++) {
			HD_EDU_1200_LCURLISTRecord rec = (HD_EDU_1200_LCURLISTRecord)ds.curlist.get(i);
			if ("NATION_FLAG".equals(rec.getCd_type())){
				int record = rx.add(depth1, "item", "");
				rx.add(record, "value", rec.cd);
				rx.add(record, "label", rec.cd_abrv_nm);
			}
		...
		for(int i = 0; i < ds.curlist.size(); i++) {
			HD_EDU_1200_LCURLISTRecord rec = (HD_EDU_1200_LCURLISTRecord)ds.curlist.get(i);
			if ("COUNTRY_FLAG".equals(rec.getCd_type())){
				int record = rx.add(depth1, "item", "");
				rx.add(record, "value", rec.cd);
				rx.add(record, "label", rec.cd_abrv_nm);
			}
