...
<div class="header">
  <a href="http://work.chosun.com/"><img src="http://image.chosun.com/img/intranet//logo_intr.png" alt="조선일보"></a>
</div>
...
<%
	...
	for(int i=-4; i<=0; i++) {
		...
		%>
				<option value="<%=dt%>" <%=(day_of_week == d && i == 0? "selected" : "")%>><%=dt.substring(4,6)%>월<%=dt.substring(6,8)%>일(<%=week_days[d-1]%>)</option>
		<%
			}
	}
%>
      ...
      <div class="option_step">
        <strong class="tit">출근시각</strong><p><input type="text" id="prsnt_tm" name="prsnt_tm" maxlength="5" placeholder="예)09:00" onfocus="$(this).select();" oninput="this.value=this.value.replace(/[^0-9,:]/g,'');chg_input(this);"></p>
      </div>
      <div class="option_step">
        <strong class="tit">퇴근시각</strong><p><input type="text" id="finish_tm" name="finish_tm" maxlength="5" placeholder="예)16:00" onfocus="$(this).select();" oninput="this.value=this.value.replace(/[^0-9,:]/g,'');chg_input(this);"></p>
      </div>
      <div class="option_step">
        <strong class="tit">추가휴게시간</strong><p><input type="text" id="addm_rest_minute" name="addm_rest_minute" maxlength="5" placeholder="예)01:00" onfocus="$(this).select();" oninput="this.value=this.value.replace(/[^0-9,:]/g,'');chg_input(this);"></p>
      </div>
      <div class="option_step">
        <input type="checkbox" id="duty_clsf" name="duty_clsf" value="10" onclick="chk_duty_clsf(this);"><label for="duty_clsf"><span>출장</span></label>
      </div>
    </div>
    ...
    <div class="btn_area">
      ...
      <button type="button" id="csSubmitButton" class="common_btn small on" onclick="search_time();"><span>조회</span></button>
    </div>
   
    <!--// 금주/ 지난주 탭추가 -->
    <span class="tbl_tit">금주 근무시간<ul class="list_sort"><li><a href="#" class="select" onclick="search_time();">금주</a></li><li class="last"><a href="#none" onclick="search_time('1');">지난주></a></li></ul></span>
    <!--// 금주/ 지난주 탭추가 -->
 
    <!--span class="tbl_tit">.금주 근무시간</span-->
    <div class="tbl_area" id="div_table" name="div_table">
      <table class="table_style" summery="금주 근무시간을 등록">
