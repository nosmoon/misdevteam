...
<%
	...
	try {
		...
		for(int i = 0; i < ds.curlist1.size(); i++) {
			HD_AFFR_2400_LCURLIST1Record rec = (HD_AFFR_2400_LCURLIST1Record)ds.curlist1.get(i);
		...
		for(int i = 0; i < ds.curlist.size(); i++) {
			HD_AFFR_2400_LCURLISTRecord rec = (HD_AFFR_2400_LCURLISTRecord)ds.curlist.get(i);
			int record = rx.add(recordSet, "record", "");
			rx.add(record, "cmpy_cd", rec.cmpy_cd);
			rx.add(record, "emp_no", rec.emp_no);
			rx.add(record, "occr_dt", rec.occr_dt);
			...
			rx.add(record, "actu_slip_no", rec.actu_slip_no);
			rx.add(record, "self_prn", rec.self_prn);
			//rx.add(record, "cmpy_cd", StringUtil.replaceToXml(rec.cmpy_cd));
			//rx.add(record, "emp_no", StringUtil.replaceToXml(rec.emp_no));
			//rx.add(record, "occr_dt", StringUtil.replaceToXml(rec.occr_dt));
			//rx.add(record, "seq", StringUtil.replaceToXml(rec.seq));
			...
			//rx.add(record, "actu_slip_no", StringUtil.replaceToXml(rec.actu_slip_no));
			//rx.add(record, "self_prn", StringUtil.replaceToXml(rec.self_prn));
		}
