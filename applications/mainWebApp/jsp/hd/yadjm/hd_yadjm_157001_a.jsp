...
<%
	/*
		파일이름 : hd_yadjm_157001_a.jsp 
		작성일자 : 2016년 1월 06일 최초작성
		작성자 : 최호정 
		내용 : 연말정산간소화 홈페이지가 홈택스로 통합됨에 따라 전자문서 API 가 신규 업데이트되어 시스템에 적용함.
	*/
	
	System.out.println("##### 15년 연말정산 파일업로드 157001_a.jsp 수행시작한다 #####");
	...
	for (int i = 0; (part = multi.readNextPart()) != null; i++) {
		if(part instanceof FilePart){
			if(((FilePart) part).getFileName() != null) {
				...
				System.out.println("##### 15년 연말정산 파일업로드 add_file ===>"+add_file);
				...
				System.out.println("##### 15년 연말정산 파일업로드 p_pwd ===>"+p_pwd);
				...
				System.out.println("##### 15년 연말정산 파일업로드 isSuccess ===>"+isSuccess);
				...
				if( isSuccess ){ //성공
	                ...
	                if ( v_ret == 1 ) {
						...
						System.out.println("##### 15년 연말정산 파일업로드 flag ===>"+flag);
	   					
						emp_no        	= Util.checkString((String)ht.get("emp_no")); //화면에서 받아온 사번
						System.out.println("##### 15년 연말정산 파일업로드 emp_no ===>"+emp_no);
					...
					System.out.println("##### 15년 연말정산 파일업로드 attcFilNm ===>"+attcFilNm);
	...
	String adjm_rvrs_yy = "2015";  //정산년도 하드코딩부분. 매년 바꿔야함(중요)
	String xml2 = "";
	String sNodeName = "";
	...
	if( isSuccess ){ //검증이 성공한 경우에만 태운다
		...
		System.out.println("##### 15년 연말정산 파일업로드 docList ===>"+docList);
					...
					if(manNode.item(iIndex).getNodeType() ==Node.ELEMENT_NODE)
					{
						...
						for(int dIndex = 0; dIndex < dataNode.getLength(); dIndex++)
						{
							...
							if(dataNode.item(dIndex).getNodeType() ==Node.ELEMENT_NODE)
							{
								...
								data = head + form_cd + man + dat_cd+"#"+busnid+"#"+trade_nm+"#"+acc_no+"#"+edu_tp+"#"+use_place_cd+"#"+donation_cd+"#"+first_tot_amt+"#"+second_tot_amt+"#"+secu_no+"#"+first_year_tot_amt+"#"+second_year_tot_amt+"#"+inqr_strt_mm+"#"+inqr_end_mm+"#";
...
%>  
<jsp:forward page="/hd/yadjm/157001">
