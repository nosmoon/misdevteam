...
<%	
	...
	System.out.println("##### 14년 연말정산 파일업로드 147001_a.jsp 수행시작한다 #####");
	...
	for (int i = 0; (part = multi.readNextPart()) != null; i++) {
		if(part instanceof FilePart){
			...
			//	TSSPdfTSTValidator.init( "/appstore/webserver/resin/cacerts", CertVerifyConst.NONE, null ); // EIP번 서버용 
			...
			//	TSSPdfTSTValidator.init( "C:/workspace/ciis_chosun_com/lib/cacerts", CertVerifyConst.NONE, null );
			//  TSSPdfTSTValidator.init( "C:/workspace/ciis_chosun_com/applications/mainWebApp/WEB-INF/lib/cacerts", CertVerifyConst.NONE, null );
				TSSPdfTSTValidator.init( "/webstore/pub/ciis/webapps/WEB-INF/lib/cacerts", CertVerifyConst.NONE, null ); //진본성검증초기화, 테스트 서버용 
			    ...
			    System.out.println("##### 14년 연말정산 파일업로드 p_pwd ===>"+p_pwd);
			    System.out.println("##### 14년 연말정산 파일업로드 add_file ===>"+add_file);
			  

				result = TSSPdfTSTValidator.validatorByte(add_file,p_pwd,p_pwd); //진본성 검증 수행
				 System.out.println("##### 14년 연말정산 파일업로드 result ===>"+result);
				...
				// 데이터 추출
					byte[] buf	= pdf.NTS_GetFileBufEx(add_file, p_pwd, "XML", false ); //XML 데이터 추출
					...
					flag       	 	= Util.checkString((String)ht.get("flag")); //화면에서 받아온 값
					System.out.println("##### 14년 연말정산 파일업로드 flag ===>"+flag);
					emp_no        	= Util.checkString((String)ht.get("emp_no")); //화면에서 받아온 사번
					System.out.println("##### 14년 연말정산 파일업로드 emp_no ===>"+emp_no);
				attcFilNm      = ((FilePart)part).getFileName();
					System.out.println("##### 14년 연말정산 파일업로드 attcFilNm ===>"+attcFilNm);
	...
	System.out.println("#####되라01");
	
	String adjm_rvrs_yy = "2014";
	String xml2 = "";
	String sNodeName = "";
	int kkkkk= 0;
	if( result == 0 ){
	//String add_file_no1  	= Util.checkString((String)ht.get("add_file_no1"));
		...
		System.out.println("되라01_1");
		NodeList docList = doc.getElementsByTagName("doc");
		System.out.println("##### 14년 연말정산 파일업로드 docList ===>"+docList);
					...
					if(manNode.item(iIndex).getNodeType() ==Node.ELEMENT_NODE)
					{
						...
						for(int dIndex = 0; dIndex < dataNode.getLength(); dIndex++)
						{
							...
							if(dataNode.item(dIndex).getNodeType() ==Node.ELEMENT_NODE)
							{
								...
								secu_no = dataElement.getAttribute("secu_no"); //2014년 연말정산 추가(기존 누락된 태그)
...
%>
<jsp:forward page="/hd/yadjm/147001">
