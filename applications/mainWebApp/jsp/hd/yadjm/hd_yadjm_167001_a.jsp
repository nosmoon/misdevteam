...
<%
	/*
		파일이름 : hd_yadjm_167001_a.jsp 
		작성일자 : 2016년 12월 27일 최초작성
		작성자 : 한현재 
		내용 : 연말정산간소화 홈페이지가 홈택스로 통합됨에 따라 전자문서 API 가 신규 업데이트되어 시스템에 적용함.
	*/
	
	System.out.println("##### 16년 연말정산 파일업로드 167001_a.jsp 수행시작한다 #####");
	...
	for (int i = 0; (part = multi.readNextPart()) != null; i++) {
		if(part instanceof FilePart){
			if(((FilePart) part).getFileName() != null) {
				...
				System.out.println("##### 16년 연말정산 파일업로드 add_file ===>"+add_file);
				...
				System.out.println("##### 16년 연말정산 파일업로드 p_pwd ===>"+p_pwd);
				...
				System.out.println("##### 16년 연말정산 파일업로드 isSuccess ===>"+isSuccess);
				...
				if( isSuccess ){ //성공
					...
					System.out.println("##### ExportCustomFile ===>");
					//3-2)XML 데이터 추출
	                ...
	                if ( v_ret == 1 ) {
						...
						System.out.println("##### 16년 연말정산 파일업로드 flag ===>"+flag);
	   					
						emp_no        	= Util.checkString((String)ht.get("emp_no")); //화면에서 받아온 사번
						System.out.println("##### 16년 연말정산 파일업로드 emp_no ===>"+emp_no);
					...
					System.out.println("##### 16년 연말정산 파일업로드 attcFilNm ===>"+attcFilNm);
	...
	String adjm_rvrs_yy = "2016";  //정산년도 하드코딩부분. 매년 바꿔야함(중요)
	String xml2 = "";
	String sNodeName = "";
	...
	if( isSuccess ){ //검증이 성공한 경우에만 태운다
		...
		System.out.println("##### 16년 연말정산 파일업로드 docList ===>"+docList);
					...
					if(manNode.item(iIndex).getNodeType() ==Node.ELEMENT_NODE)
					{
						...
						for(int dIndex = 0; dIndex < dataNode.getLength(); dIndex++)
						{
							...
							if(dNode.getNodeType() == Node.ELEMENT_NODE && form_cd.indexOf("J203Y") < 0) // 장기주택저당차입금 이자상환액은 받지않음 2017.1.17 한현재
							{
								...
								if((amtNode.getLength() != 0 && form_cd.indexOf("O101M") < 0) && (amtNode.getLength() != 0 && form_cd.indexOf("P101M") < 0))
								{	
...
%>  
<jsp:forward page="/hd/yadjm/167001">
