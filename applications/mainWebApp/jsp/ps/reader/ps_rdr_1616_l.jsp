...
<!--
	...
	function chk_search(pageno) {
	    ...
	    lo_form1.action = "/ps/reader/1621";
	    lo_form1.target = "cmprifrm";
	    lo_form1.submit();
	}

	//독자번호 위로넘김
	function rdrno_set(rdr_no,rownum,bocd){
		...
		if(lo_form.rdr_no!=null && lo_form.bocd!=null){
			...
			lo_form.cashrcptstat[rownum].value = '20';
            return;
		} else {
			...
			var inx_cashrcptstat = "cashrcptstat"+rownum;
			...
			lo_form.all[inx_cashrcptstat].value = 20;
			parent.jsDisEnabled_1(rownum);
            return;

		}
	}
/*
		if(lo_form.rdr_no!=null){
			for(var i=0;i<lo_form.rdr_no.length;i++){
				if(trim(lo_form.rdr_no[i].value)==''){
					lo_form.rdr_no[i].value = rdr_no;
					return;
				}
			}
		} else {
			for(var i=0;i<10;i++){
				var inx_nm = "rdr_no"+i;
				if(lo_form.all[inx_nm] && trim(lo_form.all[inx_nm].value)==''){
					lo_form.all[inx_nm].value = rdr_no;
					return;
				}
			}
...
*/
	...
	function change_row_color_out(obj){
	    for (i=0; i < document.all.compare_zone.rows.length; i++) {
	    	if(document.all.compare_zone.rows(i) == obj){
	            document.all.compare_zone.rows(i).style.backgroundColor = "";
                //document.all.compare_zone.rows(i).className = "out";
				if( (i%2) == 1 && document.all.compare_zone.rows(i-1) != null){	    // 짝수이면 자신보다 1 더 작은 홀수 로우까지 변경
	                document.all.compare_zone.rows(i-1).style.backgroundColor = "";
                    //document.all.compare_zone.rows(i-1).className = "out";
				}else if( (i%2) == 0 && document.all.compare_zone.rows(i+1) != null){	// 홀수이면 자신보다 1 더 큰 짝수 로우까지 변경
	                document.all.compare_zone.rows(i+1).style.backgroundColor = "";
                    //document.all.compare_zone.rows(i+1).className = "out";
				}
	        }
	...
	function change_row_color_in(obj){
	    for (i=0; i < document.all.compare_zone.rows.length; i++) {
	    	if(document.all.compare_zone.rows(i) == obj){
	            document.all.compare_zone.rows(i).style.backgroundColor = "#EDFFF1";
                //document.all.compare_zone.rows(i).className = "over";
				if( (i%2) == 1 && document.all.compare_zone.rows(i-1) != null){	    // 짝수이면 자신보다 1 더 작은 홀수 로우까지 변경
	                document.all.compare_zone.rows(i-1).style.backgroundColor = "#EDFFF1";
                    //document.all.compare_zone.rows(i-1).className = "over";
				}else if( (i%2) == 0 && document.all.compare_zone.rows(i+1) != null){	// 홀수이면 자신보다 1 더 큰 짝수 로우까지 변경
	                document.all.compare_zone.rows(i+1).style.backgroundColor = "#EDFFF1";
                    //document.all.compare_zone.rows(i+1).className = "over";
				}
...
<!--검색끝-->
<!--list start -->
<table width="830" border="0" align="center" cellpadding="1" cellspacing="0" bgcolor="#ffffff" class="gray" id="compare_zone">
