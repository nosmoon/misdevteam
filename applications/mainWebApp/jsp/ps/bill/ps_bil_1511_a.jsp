...
<%
	...
	//String	IP		= "210.181.29.11"; //테스트
	String	IP		= "210.181.29.21"; 운영
	...
	String tid		= "DPT0022016";		//	Test 단말기 번호 - 향후 단말기 번호 변경 요망 => 이체청구시 단말기 번호와 동일
	...
	KSMpiIspBean.argSetInitLog("/was/publishdept/log/ksnet/log","Y","/was/publishdept/log/ksnet/errlog","Y");
...
<%
	if(status.equals("O")) {	// 승인요청 결과가 정상일 경우
         if( issue.equals( request.getParameter("cardcmpycd") ) ) {
%>
...
%>
         <script language="javascript">
           alert("카드종류(카드사)가 카드번호와 일치하지 않습니다.\n다시 시도해 주세요.");
         </script>
<%
        }
%>
...
<script language="javascript">
	<%
		if(_error.equals("")) {
			String msg = (msg1+msg2+msg3+msg4).trim();
	%>
	var ls_error_msg = "다시 시도해 주세요." + "\n" + "(<%=msg%>)";
	<%
		} else {
	%>
	var ls_error_msg = "<%=_error%>" + "\n" + "다시 시도해 주세요.";
	<%
		}
	%>
	alert(ls_error_msg);
