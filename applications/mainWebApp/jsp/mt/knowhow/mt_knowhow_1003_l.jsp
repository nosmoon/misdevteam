<%@	page contentType="text/xml; charset=EUC-KR" %>

<%@	page import="
		java.io.*
	,	java.sql.*
	,	java.util.*
	,	kr.co.comsquare.rwXmlLib.*
	,	kr.co.comsquare.db.*
	,	kr.co.comsquare.util.Tokenizer
	,	chosun.ciis.co.base.util.*
	,	chosun.ciis.mt.knowhow.rec.*
	,	chosun.ciis.mt.knowhow.ds.*;
	"
%>

<%
	RwXml rx = new RwXml();
	MT_KNOWHOW_1003_LDataSet ds = (MT_KNOWHOW_1003_LDataSet)request.getAttribute("ds");
	int root = RwXml.rootNodeID;
	int dataSet = 0;
	int recordSet = 0;
	String errcode = ds.errcode;
	String errmsg = ds.errmsg;
	dataSet = rx.add(root, "dataSet", "");

	try {
		/****** CURLIST2 BEGIN */
		recordSet = rx.add(dataSet, "CURLIST2", "");

		for(int i = 0; i < ds.curlist2.size(); i++) {
			MT_KNOWHOW_1003_LCURLIST2Record rec = (MT_KNOWHOW_1003_LCURLIST2Record)ds.curlist2.get(i);
			int record = rx.add(recordSet, "record", "");
			rx.add(record, "reg_dt", StringUtil.replaceToXml(rec.reg_dt));
			rx.add(record, "seq", StringUtil.replaceToXml(rec.seq));
			rx.add(record, "make_pers_emp_nm", StringUtil.replaceToXml(rec.make_pers_emp_nm));
			rx.add(record, "qust_answer", StringUtil.replaceToXml(rec.qust_answer));
			//rx.add(record, "reg_dt", rec.reg_dt);
			//rx.add(record, "seq", rec.seq);
			//rx.add(record, "make_pers_emp_nm", rec.make_pers_emp_nm);
			//rx.add(record, "qust_answer", rec.qust_answer);
		}
		rx.add(recordSet, "totalcnt", ds.curlist2.size());
		/****** CURLIST2 END */

		/****** CURLIST1 BEGIN */
		recordSet = rx.add(dataSet, "CURLIST1", "");

		for(int i = 0; i < ds.curlist1.size(); i++) {
			MT_KNOWHOW_1003_LCURLIST1Record rec = (MT_KNOWHOW_1003_LCURLIST1Record)ds.curlist1.get(i);
			int record = rx.add(recordSet, "record", "");
			rx.add(record, "reg_dt", StringUtil.replaceToXml(rec.reg_dt));
			rx.add(record, "lcls", StringUtil.replaceToXml(rec.lcls));
			rx.add(record, "reg_seq", StringUtil.replaceToXml(rec.reg_seq));
			rx.add(record, "qust_answer", StringUtil.replaceToXml(rec.qust_answer));
			rx.add(record, "make_pers_emp_nm", StringUtil.replaceToXml(rec.make_pers_emp_nm));
			//rx.add(record, "reg_dt", rec.reg_dt);
			//rx.add(record, "lcls", rec.lcls);
			//rx.add(record, "reg_seq", rec.reg_seq);
			//rx.add(record, "qust_answer", rec.qust_answer);
			//rx.add(record, "make_pers_emp_nm", rec.make_pers_emp_nm);
		}
		rx.add(recordSet, "totalcnt", ds.curlist1.size());
		/****** CURLIST1 END */

	}
	catch (Exception e) {
		errcode += " JSP Error";
		errmsg += " JSP Error Message = " + e.getMessage();
	}
	finally {
		rx.add(dataSet, "errcode", errcode);
		rx.add(dataSet, "errmsg", errmsg);
		out.println(rx.xmlFlush());
		out.println(rx.xmlEndFlush());
	}
%>

<%
/*
TrustForm의 Instance 선언부에 복사해서 사용
<mt_knowhow_1003_l>
	<dataSet>
		<CURLIST2>
			<record>
				<reg_dt/>
				<seq/>
				<make_pers_emp_nm/>
				<qust_answer/>
			</record>
		</CURLIST2>
	</dataSet>
</mt_knowhow_1003_l>
*/
%>

<%
/*
TrustForm의 Instance 선언부에 복사해서 사용
<mt_knowhow_1003_l>
	<dataSet>
		<CURLIST1>
			<record>
				<reg_dt/>
				<lcls/>
				<reg_seq/>
				<qust_answer/>
				<make_pers_emp_nm/>
			</record>
		</CURLIST1>
	</dataSet>
</mt_knowhow_1003_l>
*/
%>

<% /* 작성시간 : Wed Jun 10 16:29:15 KST 2009 */ %>