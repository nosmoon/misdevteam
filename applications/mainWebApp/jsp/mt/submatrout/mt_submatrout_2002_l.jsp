...
<%
	...
	try {
		...
		for(int i = 0; i < ds.curlist.size(); i++) {
			MT_SUBMATROUT_2002_LCURLISTRecord rec = (MT_SUBMATROUT_2002_LCURLISTRecord)ds.curlist.get(i);
			...
			rx.add(record, "mov_out_budg_acctcd", rec.mov_out_budg_acctcd);
			rx.add(record, "mov_out_budg_cd_nm", rec.mov_out_budg_cd_nm);
			rx.add(record, "mov_out_budg_acctcdnm", rec.mov_out_budg_acctcdnm);
			rx.add(record, "mov_out_qunt", rec.mov_out_qunt);
			rx.add(record, "mov_out_unit", rec.mov_out_unit);
			rx.add(record, "mov_out_uprc", rec.mov_out_uprc);
			rx.add(record, "mov_out_amt", rec.mov_out_amt);
			...
			rx.add(record, "mov_in_budg_acctcd", rec.mov_in_budg_acctcd);
			rx.add(record, "mov_in_budg_cd_nm", rec.mov_in_budg_cd_nm);
			rx.add(record, "mov_in_budg_acctcdnm", rec.mov_in_budg_acctcdnm);
			rx.add(record, "mov_in_qunt", rec.mov_in_qunt);
			rx.add(record, "mov_in_unit", rec.mov_in_unit);
			rx.add(record, "mov_in_uprc", rec.mov_in_uprc);
			rx.add(record, "mov_in_amt", rec.mov_in_amt);
			rx.add(record, "medi_cd", rec.medi_cd);
			rx.add(record, "medi_cd_nm", rec.medi_cd_nm);
			rx.add(record, "medi_ser_no", rec.medi_ser_no);
			rx.add(record, "remk", rec.remk);
			rx.add(record, "hjg_cnt_minus_out_qunt" , Long.parseLong(rec.hjg_cnt) + Long.parseLong(rec.mov_out_qunt));
			rx.add(record, "cut_hjg_amt", "");
...
<% /* 작성시간 : Mon Mar 30 17:03:31 KST 2009 */ %>