...
<%
	RwXml rx = new RwXml();
	MT_PAPINOUT_5104_LDataSet ds = (MT_PAPINOUT_5104_LDataSet)request.getAttribute("ds");
	...
	try {
		...
		for(int i = 0; i < ds.curlist.size(); i++) {
			MT_PAPINOUT_5104_LCURLISTRecord rec = (MT_PAPINOUT_5104_LCURLISTRecord)ds.curlist.get(i);
			...
			rx.add(record, "pj_qunt", rec.qunt);
			rx.add(record, "pj_wgt", rec.wgt);
			rx.add(record, "uprc", rec.uprc);
			rx.add(record, "amt", rec.amt);
			
			
			iQuntTot = iQuntTot + Integer.parseInt(rec.qunt);
			iAmtTot  = iAmtTot  + Integer.parseInt(rec.amt);
			iWgtTot  = iWgtTot  + Integer.parseInt(rec.wgt);
