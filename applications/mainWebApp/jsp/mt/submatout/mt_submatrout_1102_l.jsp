...
<%
	...
	try {
		...
		for(int i = 0; i < ds.curlist2.size(); i++) {
			...
			rx.add(record, "yesday_cnt", rec.hjg_cnt);
			rx.add(record, "hjg_cnt",   0);
		...
		for(int i = 0; i < ds.curlist.size(); i++) {
			MT_SUBMATROUT_1102_LCURLISTRecord rec = (MT_SUBMATROUT_1102_LCURLISTRecord)ds.curlist.get(i);
			int record = rx.add(recordSet, "record", "");
			rx.add(record, "seq", "");
			rx.add(record, "matr_cd", rec.matr_cd);
			rx.add(record, "matr_nm", rec.matr_nm);
			rx.add(record, "unit", rec.unit);
			rx.add(record, "yesday_cnt", Long.parseLong(rec.hjg_cnt) + Long.parseLong(rec.prv_cnt));
			rx.add(record, "hjg_cnt",   0);
			rx.add(record, "owh_qunt",  0);
			rx.add(record, "owh_uprc", rec.maeipdan);
			rx.add(record, "owh_amt",   0);
