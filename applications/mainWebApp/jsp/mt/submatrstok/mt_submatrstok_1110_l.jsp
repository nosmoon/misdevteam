...
<%
	RwXml rx = new RwXml();
	MT_SUBMATRSTOK_1110_LDataSet ds = (MT_SUBMATRSTOK_1110_LDataSet)request.getAttribute("ds");
	...
	try {
		...
		for(int i = 0; i < ds.curlist2.size(); i++) {
			MT_SUBMATRSTOK_1110_LCURLIST2Record rec = (MT_SUBMATRSTOK_1110_LCURLIST2Record)ds.curlist2.get(i);
			...
			rx.add(record, "befor_roll", rec.befor_roll);
			rx.add(record, "ewh_roll", rec.ewh_roll);
			rx.add(record, "use_roll", rec.use_roll);
			rx.add(record, "jego_roll", rec.jego_roll);
		...
		for(int i = 0; i < ds.curlist1.size(); i++) {
			MT_SUBMATRSTOK_1110_LCURLIST1Record rec = (MT_SUBMATRSTOK_1110_LCURLIST1Record)ds.curlist1.get(i);
			int record = rx.add(recordSet, "record", "");
			rx.add(record, "cmpy_cd", rec.cmpy_cd);
			rx.add(record, "occr_dt", rec.occr_dt);
			rx.add(record, "seq", rec.seq);
