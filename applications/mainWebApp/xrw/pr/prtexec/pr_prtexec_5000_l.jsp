...
<%
	...
	try {
		...
		recordSet = rx.add(dataSet, "clos_yn", ds.clos_yn);
		/*
		for(int i = 0; i < ds.curlist.size(); i++) {
			PR_PRTEXEC_5000_LCURLISTRecord rec = (PR_PRTEXEC_5000_LCURLISTRecord)ds.curlist.get(i);
			int record = rx.add(recordSet, "record", "");
			rx.add(record, "cmpy_cd", rec.cmpy_cd);
			rx.add(record, "occr_dt", rec.occr_dt);
			rx.add(record, "seq", rec.seq);
			...
			rx.add(record, "is_cnt_ex_yn", rec.is_cnt_ex_yn);
			rx.add(record, "is_qty_inc_yn", rec.is_qty_inc_yn);
			rx.add(record, "add_prt_yn", rec.add_prt_yn);
