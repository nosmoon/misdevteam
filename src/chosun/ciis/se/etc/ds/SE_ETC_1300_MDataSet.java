...
public class SE_ETC_1300_MDataSet extends somo.framework.db.BaseDataSet implements java.io.Serializable{
	...
	public SE_ETC_1300_MDataSet(){}
	public SE_ETC_1300_MDataSet(String errcode, String errmsg, String yymm){
	...
	public void getValues(CallableStatement cstmt) throws SQLException{
		...
		while(rset0.next()){
			SE_ETC_1300_MTEAMLISTRecord rec = new SE_ETC_1300_MTEAMLISTRecord();
		...
		while(rset1.next()){
			SE_ETC_1300_MPARTLISTRecord rec = new SE_ETC_1300_MPARTLISTRecord();
		...
		while(rset2.next()){
			SE_ETC_1300_MAREALISTRecord rec = new SE_ETC_1300_MAREALISTRecord();
		...
		while(rset3.next()){
			SE_ETC_1300_MSELLCLSFLISTRecord rec = new SE_ETC_1300_MSELLCLSFLISTRecord();
...
<%
	for(int i=0; i<ds.teamlist.size(); i++){
		SE_ETC_1300_MTEAMLISTRecord teamlistRec = (SE_ETC_1300_MTEAMLISTRecord)ds.teamlist.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.partlist.size(); i++){
		SE_ETC_1300_MPARTLISTRecord partlistRec = (SE_ETC_1300_MPARTLISTRecord)ds.partlist.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.arealist.size(); i++){
		SE_ETC_1300_MAREALISTRecord arealistRec = (SE_ETC_1300_MAREALISTRecord)ds.arealist.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.sellclsflist.size(); i++){
		SE_ETC_1300_MSELLCLSFLISTRecord sellclsflistRec = (SE_ETC_1300_MSELLCLSFLISTRecord)ds.sellclsflist.get(i);%>
HTML 코드들....
	<%}%>
...
/* 작성시간 : Thu May 28 14:38:58 KST 2015 */