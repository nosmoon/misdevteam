...
public class SE_RCP_2500_MDataSet extends somo.framework.db.BaseDataSet implements java.io.Serializable{
	...
	public SE_RCP_2500_MDataSet(){}
	public SE_RCP_2500_MDataSet(String errcode, String errmsg, String yymm){
	...
	public void getValues(CallableStatement cstmt) throws SQLException{
		...
		while(rset3.next()){
			SE_RCP_2500_MTEAM_CDCURRecord rec = new SE_RCP_2500_MTEAM_CDCURRecord();
		...
		while(rset4.next()){
			SE_RCP_2500_MPART_CDCURRecord rec = new SE_RCP_2500_MPART_CDCURRecord();
		...
		while(rset5.next()){
			SE_RCP_2500_MAREA_CDCURRecord rec = new SE_RCP_2500_MAREA_CDCURRecord();
...
<%
	SE_RCP_2500_MDataSet ds = (SE_RCP_2500_MDataSet)request.getAttribute("ds");
%>
Web Tier에서 Record 객체 관련 코드 작성시 사용하십시오.

<%
	for(int i=0; i<ds.team_cdcur.size(); i++){
		SE_RCP_2500_MTEAM_CDCURRecord team_cdcurRec = (SE_RCP_2500_MTEAM_CDCURRecord)ds.team_cdcur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.part_cdcur.size(); i++){
		SE_RCP_2500_MPART_CDCURRecord part_cdcurRec = (SE_RCP_2500_MPART_CDCURRecord)ds.part_cdcur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.area_cdcur.size(); i++){
		SE_RCP_2500_MAREA_CDCURRecord area_cdcurRec = (SE_RCP_2500_MAREA_CDCURRecord)ds.area_cdcur.get(i);%>
HTML 코드들....
	<%}%>
