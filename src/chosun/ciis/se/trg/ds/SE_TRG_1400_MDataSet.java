...
public class SE_TRG_1400_MDataSet extends somo.framework.db.BaseDataSet implements java.io.Serializable{
	...
	public SE_TRG_1400_MDataSet(){}
	public SE_TRG_1400_MDataSet(String errcode, String errmsg, long colcnt){
	...
	public void getValues(CallableStatement cstmt) throws SQLException{
		...
		while(rset0.next()){
			SE_TRG_1400_MTEAM_CDCURRecord rec = new SE_TRG_1400_MTEAM_CDCURRecord();
		...
		while(rset1.next()){
			SE_TRG_1400_MPART_CDCURRecord rec = new SE_TRG_1400_MPART_CDCURRecord();
		...
		while(rset2.next()){
			SE_TRG_1400_MAREA_CDCURRecord rec = new SE_TRG_1400_MAREA_CDCURRecord();
		...
		while(rset3.next()){
			SE_TRG_1400_MCOMMCDCUR1Record rec = new SE_TRG_1400_MCOMMCDCUR1Record();
...
<%
	for(int i=0; i<ds.team_cdcur.size(); i++){
		SE_TRG_1400_MTEAM_CDCURRecord team_cdcurRec = (SE_TRG_1400_MTEAM_CDCURRecord)ds.team_cdcur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.part_cdcur.size(); i++){
		SE_TRG_1400_MPART_CDCURRecord part_cdcurRec = (SE_TRG_1400_MPART_CDCURRecord)ds.part_cdcur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.area_cdcur.size(); i++){
		SE_TRG_1400_MAREA_CDCURRecord area_cdcurRec = (SE_TRG_1400_MAREA_CDCURRecord)ds.area_cdcur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.commcdcur1.size(); i++){
		SE_TRG_1400_MCOMMCDCUR1Record commcdcur1Rec = (SE_TRG_1400_MCOMMCDCUR1Record)ds.commcdcur1.get(i);%>
HTML 코드들....
	<%}%>
