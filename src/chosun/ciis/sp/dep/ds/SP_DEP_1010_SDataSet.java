...
public class SP_DEP_1010_SDataSet extends somo.framework.db.BaseDataSet implements java.io.Serializable{
	...
	public SP_DEP_1010_SDataSet(){}
	public SP_DEP_1010_SDataSet(String errcode, String errmsg, String occr_dt, String slip_clsf, String seq, String rcpm_shet_no, String self_cmpy_yn, String rcpm_plac_clsf, String dlco_no, String dlco_nm, String medi_cd, String dept_cd, String sub_dept_cd, String mchrg_pers, String mchrg_pers_nm, String clos_yn, String seq_clsf, String pre_seq, String next_seq){
	...
	public void getValues(CallableStatement cstmt) throws SQLException{
		...
		this.slip_clsf = Util.checkString(cstmt.getString(5));
		this.seq = Util.checkString(cstmt.getString(6));
		this.rcpm_shet_no = Util.checkString(cstmt.getString(7));
		this.self_cmpy_yn = Util.checkString(cstmt.getString(8));
		this.rcpm_plac_clsf = Util.checkString(cstmt.getString(9));
		this.dlco_no = Util.checkString(cstmt.getString(10));
		this.dlco_nm = Util.checkString(cstmt.getString(11));
		this.medi_cd = Util.checkString(cstmt.getString(12));
		this.dept_cd = Util.checkString(cstmt.getString(13));
		this.sub_dept_cd = Util.checkString(cstmt.getString(14));
		this.mchrg_pers = Util.checkString(cstmt.getString(15));
		this.mchrg_pers_nm = Util.checkString(cstmt.getString(16));
		this.clos_yn = Util.checkString(cstmt.getString(17));
		this.seq_clsf = Util.checkString(cstmt.getString(18));
		this.pre_seq = Util.checkString(cstmt.getString(19));
		this.next_seq = Util.checkString(cstmt.getString(20));
		ResultSet rset0 = (ResultSet) cstmt.getObject(21);
		while(rset0.next()){
			SP_DEP_1010_SCURLIST1Record rec = new SP_DEP_1010_SCURLIST1Record();
		...
		while(rset1.next()){
			SP_DEP_1010_SCURLIST2Record rec = new SP_DEP_1010_SCURLIST2Record();
			...
			rec.cash_rcpt_use_yn = Util.checkString(rset1.getString("cash_rcpt_use_yn"));
			rec.cash_rcpt_acq_yn = Util.checkString(rset1.getString("cash_rcpt_acq_yn"));
			rec.deal_cd = Util.checkString(rset1.getString("deal_cd"));
			rec.tmnl_no = Util.checkString(rset1.getString("tmnl_no"));
			rec.aprv_no = Util.checkString(rset1.getString("aprv_no"));
			rec.card_aprv_dd = Util.checkString(rset1.getString("card_aprv_dd"));
			rec.decid_no = Util.checkString(rset1.getString("decid_no"));
			rec.card_rcpm_yn = Util.checkString(rset1.getString("card_rcpm_yn"));
...
<%
	SP_DEP_1010_SDataSet ds = (SP_DEP_1010_SDataSet)request.getAttribute("ds");
%>
Web Tier에서 Record 객체 관련 코드 작성시 사용하십시오.

<%
	for(int i=0; i<ds.curlist1.size(); i++){
		SP_DEP_1010_SCURLIST1Record curlist1Rec = (SP_DEP_1010_SCURLIST1Record)ds.curlist1.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.curlist2.size(); i++){
		SP_DEP_1010_SCURLIST2Record curlist2Rec = (SP_DEP_1010_SCURLIST2Record)ds.curlist2.get(i);%>
HTML 코드들....
	<%}%>
...
<%= curlist2Rec.cash_rcpt_use_yn%>
<%= curlist2Rec.cash_rcpt_acq_yn%>
<%= curlist2Rec.deal_cd%>
<%= curlist2Rec.tmnl_no%>
<%= curlist2Rec.aprv_no%>
<%= curlist2Rec.card_aprv_dd%>
<%= curlist2Rec.decid_no%>
<%= curlist2Rec.card_rcpm_yn%>
<%= curlist2Rec.card_mang_no%>
----------------------------------------------------------------------------------------------------*/


/* 작성시간 : Wed Jul 25 19:42:53 KST 2012 */