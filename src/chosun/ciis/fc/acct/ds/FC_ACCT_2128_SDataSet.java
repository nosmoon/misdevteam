...
public class FC_ACCT_2128_SDataSet extends somo.framework.db.BaseDataSet implements java.io.Serializable{
	...
	public FC_ACCT_2128_SDataSet(){}
	public FC_ACCT_2128_SDataSet(String errcode, String errmsg){
	...
	public void getValues(CallableStatement cstmt) throws SQLException{
		...
		while(rset0.next()){
			FC_ACCT_2128_SCURLISTRecord rec = new FC_ACCT_2128_SCURLISTRecord();
			rec.rcpm_amt = Util.checkString(rset0.getString("rcpm_amt"));
			this.curlist.add(rec);
		}
		ResultSet rset1 = (ResultSet) cstmt.getObject(10);
		while(rset1.next()){
			FC_ACCT_2128_SCURLIST1Record rec = new FC_ACCT_2128_SCURLIST1Record();
		...
		while(rset2.next()){
			FC_ACCT_2128_SCURLIST2Record rec = new FC_ACCT_2128_SCURLIST2Record();
		...
		while(rset3.next()){
			FC_ACCT_2128_SCURLIST3Record rec = new FC_ACCT_2128_SCURLIST3Record();
		...
		while(rset4.next()){
			FC_ACCT_2128_SCURLIST4Record rec = new FC_ACCT_2128_SCURLIST4Record();
			...
			rec.rcpm_slip_no = Util.checkString(rset4.getString("rcpm_slip_no"));
...
<%
	FC_ACCT_2128_SDataSet ds = (FC_ACCT_2128_SDataSet)request.getAttribute("ds");
%>
Web Tier에서 Record 객체 관련 코드 작성시 사용하십시오.

<%
	for(int i=0; i<ds.curlist.size(); i++){
		FC_ACCT_2128_SCURLISTRecord curlistRec = (FC_ACCT_2128_SCURLISTRecord)ds.curlist.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.curlist1.size(); i++){
		FC_ACCT_2128_SCURLIST1Record curlist1Rec = (FC_ACCT_2128_SCURLIST1Record)ds.curlist1.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.curlist2.size(); i++){
		FC_ACCT_2128_SCURLIST2Record curlist2Rec = (FC_ACCT_2128_SCURLIST2Record)ds.curlist2.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.curlist3.size(); i++){
		FC_ACCT_2128_SCURLIST3Record curlist3Rec = (FC_ACCT_2128_SCURLIST3Record)ds.curlist3.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.curlist4.size(); i++){
		FC_ACCT_2128_SCURLIST4Record curlist4Rec = (FC_ACCT_2128_SCURLIST4Record)ds.curlist4.get(i);%>
HTML 코드들....
	<%}%>
