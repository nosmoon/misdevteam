...
public class FC_ACCT_2610_MDataSet extends somo.framework.db.BaseDataSet implements java.io.Serializable{
	...
	public FC_ACCT_2610_MDataSet(){}
	public FC_ACCT_2610_MDataSet(String errcode, String errmsg){
	...
	public void getValues(CallableStatement cstmt) throws SQLException{
		...
		while(rset0.next()){
			FC_ACCT_2610_MCUR_DLCO_LISTRecord rec = new FC_ACCT_2610_MCUR_DLCO_LISTRecord();
		...
		while(rset1.next()){
			FC_ACCT_2610_MCUR_CLSF_LISTRecord rec = new FC_ACCT_2610_MCUR_CLSF_LISTRecord();
		...
		while(rset2.next()){
			FC_ACCT_2610_MCUR_SLIP_CLSF_LISTRecord rec = new FC_ACCT_2610_MCUR_SLIP_CLSF_LISTRecord();
...
<%
	FC_ACCT_2610_MDataSet ds = (FC_ACCT_2610_MDataSet)request.getAttribute("ds");
%>
Web Tier에서 Record 객체 관련 코드 작성시 사용하십시오.

<%
	for(int i=0; i<ds.cur_dlco_list.size(); i++){
		FC_ACCT_2610_MCUR_DLCO_LISTRecord cur_dlco_listRec = (FC_ACCT_2610_MCUR_DLCO_LISTRecord)ds.cur_dlco_list.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.cur_clsf_list.size(); i++){
		FC_ACCT_2610_MCUR_CLSF_LISTRecord cur_clsf_listRec = (FC_ACCT_2610_MCUR_CLSF_LISTRecord)ds.cur_clsf_list.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.cur_slip_clsf_list.size(); i++){
		FC_ACCT_2610_MCUR_SLIP_CLSF_LISTRecord cur_slip_clsf_listRec = (FC_ACCT_2610_MCUR_SLIP_CLSF_LISTRecord)ds.cur_slip_clsf_list.get(i);%>
HTML 코드들....
	<%}%>
