...
public class PL_SAL_1400_MDataSet extends somo.framework.db.BaseDataSet implements java.io.Serializable{
	...
	public PL_SAL_1400_MDataSet(){}
	public PL_SAL_1400_MDataSet(String errcode, String errmsg){
	...
	public void getValues(CallableStatement cstmt) throws SQLException{
		...
		while(rset1.next()){
			PL_SAL_1400_MMEDI_CLSFCURRecord rec = new PL_SAL_1400_MMEDI_CLSFCURRecord();
		...
		while(rset2.next()){
			PL_SAL_1400_MMEDI_CDCURRecord rec = new PL_SAL_1400_MMEDI_CDCURRecord();
		...
		while(rset3.next()){
			PL_SAL_1400_MSALE_CLSFCURRecord rec = new PL_SAL_1400_MSALE_CLSFCURRecord();
		...
		while(rset4.next()){
			PL_SAL_1400_MSENDCLSFCURRecord rec = new PL_SAL_1400_MSENDCLSFCURRecord();
		...
		while(rset5.next()){
			PL_SAL_1400_MWH_CDCURRecord rec = new PL_SAL_1400_MWH_CDCURRecord();
		...
		while(rset6.next()){
			PL_SAL_1400_MSELL_TYPECURRecord rec = new PL_SAL_1400_MSELL_TYPECURRecord();
		...
		while(rset7.next()){
			PL_SAL_1400_MORDR_PTCRCURRecord rec = new PL_SAL_1400_MORDR_PTCRCURRecord();
			rec.ordr_dt = Util.checkString(rset7.getString("ordr_dt"));
			rec.ordr_grp_seq = Util.checkString(rset7.getString("ordr_grp_seq"));
			rec.ordr_seq = Util.checkString(rset7.getString("ordr_seq"));
...
<%
	PL_SAL_1400_MDataSet ds = (PL_SAL_1400_MDataSet)request.getAttribute("ds");
%>
Web Tier에서 Record 객체 관련 코드 작성시 사용하십시오.

<%
	for(int i=0; i<ds.medi_clsfcur.size(); i++){
		PL_SAL_1400_MMEDI_CLSFCURRecord medi_clsfcurRec = (PL_SAL_1400_MMEDI_CLSFCURRecord)ds.medi_clsfcur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.medi_cdcur.size(); i++){
		PL_SAL_1400_MMEDI_CDCURRecord medi_cdcurRec = (PL_SAL_1400_MMEDI_CDCURRecord)ds.medi_cdcur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.sale_clsfcur.size(); i++){
		PL_SAL_1400_MSALE_CLSFCURRecord sale_clsfcurRec = (PL_SAL_1400_MSALE_CLSFCURRecord)ds.sale_clsfcur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.sendclsfcur.size(); i++){
		PL_SAL_1400_MSENDCLSFCURRecord sendclsfcurRec = (PL_SAL_1400_MSENDCLSFCURRecord)ds.sendclsfcur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.wh_cdcur.size(); i++){
		PL_SAL_1400_MWH_CDCURRecord wh_cdcurRec = (PL_SAL_1400_MWH_CDCURRecord)ds.wh_cdcur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.sell_typecur.size(); i++){
		PL_SAL_1400_MSELL_TYPECURRecord sell_typecurRec = (PL_SAL_1400_MSELL_TYPECURRecord)ds.sell_typecur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.ordr_ptcrcur.size(); i++){
		PL_SAL_1400_MORDR_PTCRCURRecord ordr_ptcrcurRec = (PL_SAL_1400_MORDR_PTCRCURRecord)ds.ordr_ptcrcur.get(i);%>
HTML 코드들....
	<%}%>
