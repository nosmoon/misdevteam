...
public class PL_RTN_1200_MDataSet extends somo.framework.db.BaseDataSet implements java.io.Serializable{
	...
	public PL_RTN_1200_MDataSet(){}
	public PL_RTN_1200_MDataSet(String errcode, String errmsg){
	...
	public void getValues(CallableStatement cstmt) throws SQLException{
		...
		while(rset6.next()){
			PL_RTN_1200_MMEDI_CDCURRecord rec = new PL_RTN_1200_MMEDI_CDCURRecord();
		...
		while(rset7.next()){
			PL_RTN_1200_MWH_CDCURRecord rec = new PL_RTN_1200_MWH_CDCURRecord();
		...
		while(rset8.next()){
			PL_RTN_1200_MDEAL_PTCRCURRecord rec = new PL_RTN_1200_MDEAL_PTCRCURRecord();
...
<%
	PL_RTN_1200_MDataSet ds = (PL_RTN_1200_MDataSet)request.getAttribute("ds");
%>
Web Tier에서 Record 객체 관련 코드 작성시 사용하십시오.

<%
	for(int i=0; i<ds.medi_cdcur.size(); i++){
		PL_RTN_1200_MMEDI_CDCURRecord medi_cdcurRec = (PL_RTN_1200_MMEDI_CDCURRecord)ds.medi_cdcur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.wh_cdcur.size(); i++){
		PL_RTN_1200_MWH_CDCURRecord wh_cdcurRec = (PL_RTN_1200_MWH_CDCURRecord)ds.wh_cdcur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.deal_ptcrcur.size(); i++){
		PL_RTN_1200_MDEAL_PTCRCURRecord deal_ptcrcurRec = (PL_RTN_1200_MDEAL_PTCRCURRecord)ds.deal_ptcrcur.get(i);%>
HTML 코드들....
	<%}%>
