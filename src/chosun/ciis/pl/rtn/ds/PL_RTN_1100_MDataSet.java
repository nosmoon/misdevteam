...
public class PL_RTN_1100_MDataSet extends somo.framework.db.BaseDataSet implements java.io.Serializable{
	...
	public PL_RTN_1100_MDataSet(){}
	public PL_RTN_1100_MDataSet(String errcode, String errmsg){
	...
	public void getValues(CallableStatement cstmt) throws SQLException{
		...
		while(rset9.next()){
			PL_RTN_1100_MMEDI_CDCURRecord rec = new PL_RTN_1100_MMEDI_CDCURRecord();
		...
		while(rset10.next()){
			PL_RTN_1100_MWH_CDCURRecord rec = new PL_RTN_1100_MWH_CDCURRecord();
		...
		while(rset11.next()){
			PL_RTN_1100_MDEAL_PTCRCURRecord rec = new PL_RTN_1100_MDEAL_PTCRCURRecord();
			rec.deal_dt = Util.checkString(rset11.getString("deal_dt"));
			rec.deal_grp_seq = Util.checkString(rset11.getString("deal_grp_seq"));
			rec.deal_seq = Util.checkString(rset11.getString("deal_seq"));
			...
			rec.deal_uprc = Util.checkString(rset11.getString("deal_uprc"));
			rec.dcrate = Util.checkString(rset11.getString("dcrate"));
			rec.deal_amt = Util.checkString(rset11.getString("deal_amt"));
			rec.wh_cd = Util.checkString(rset11.getString("wh_cd"));
			rec.prv_deli_qty = Util.checkString(rset11.getString("prv_deli_qty"));
			rec.prv_widr_qty = Util.checkString(rset11.getString("prv_widr_qty"));
			rec.prv_retn_qty = Util.checkString(rset11.getString("prv_retn_qty"));
...
<%
	PL_RTN_1100_MDataSet ds = (PL_RTN_1100_MDataSet)request.getAttribute("ds");
%>
Web Tier에서 Record 객체 관련 코드 작성시 사용하십시오.

<%
	for(int i=0; i<ds.medi_cdcur.size(); i++){
		PL_RTN_1100_MMEDI_CDCURRecord medi_cdcurRec = (PL_RTN_1100_MMEDI_CDCURRecord)ds.medi_cdcur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.wh_cdcur.size(); i++){
		PL_RTN_1100_MWH_CDCURRecord wh_cdcurRec = (PL_RTN_1100_MWH_CDCURRecord)ds.wh_cdcur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.deal_ptcrcur.size(); i++){
		PL_RTN_1100_MDEAL_PTCRCURRecord deal_ptcrcurRec = (PL_RTN_1100_MDEAL_PTCRCURRecord)ds.deal_ptcrcur.get(i);%>
HTML 코드들....
	<%}%>
