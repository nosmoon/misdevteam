...
public class PL_COM_1010_LDataSet extends somo.framework.db.BaseDataSet implements java.io.Serializable{
	public ArrayList dlcoinfolist = new ArrayList();
	public String errcode;
	public String errmsg;

	public PL_COM_1010_LDataSet(){}
	public PL_COM_1010_LDataSet(String errcode, String errmsg){
	...
	public void getValues(CallableStatement cstmt) throws SQLException{
		...
		while(rset3.next()){
			PL_COM_1010_LDLCOINFOLISTRecord rec = new PL_COM_1010_LDLCOINFOLISTRecord();
			...
			rec.dlco_abrv_nm = Util.checkString(rset3.getString("dlco_abrv_nm"));
			rec.comn_mang_no = Util.checkString(rset3.getString("comn_mang_no"));
			rec.rptv_flnm = Util.checkString(rset3.getString("rptv_flnm"));
			rec.dlco_clsf = Util.checkString(rset3.getString("dlco_clsf"));
			rec.dlco_clsf_nm = Util.checkString(rset3.getString("dlco_clsf_nm"));
			rec.area = Util.checkString(rset3.getString("area"));
			rec.area_nm = Util.checkString(rset3.getString("area_nm"));
			rec.prv_curr_clsf = Util.checkString(rset3.getString("prv_curr_clsf"));
			rec.prv_curr_clsf_nm = Util.checkString(rset3.getString("prv_curr_clsf_nm"));
			rec.tel_no = Util.checkString(rset3.getString("tel_no"));
			rec.ptph_no = Util.checkString(rset3.getString("ptph_no"));
...
<%
	PL_COM_1010_LDataSet ds = (PL_COM_1010_LDataSet)request.getAttribute("ds");
%>
Web Tier에서 Record 객체 관련 코드 작성시 사용하십시오.

<%
	for(int i=0; i<ds.dlcoinfolist.size(); i++){
		PL_COM_1010_LDLCOINFOLISTRecord dlcoinfolistRec = (PL_COM_1010_LDLCOINFOLISTRecord)ds.dlcoinfolist.get(i);%>
HTML 코드들....
	<%}%>
...
/* 작성시간 : Fri Mar 06 19:54:52 KST 2009 */