...
public class PL_COM_1000_MDataSet extends somo.framework.db.BaseDataSet implements java.io.Serializable{
	...
	public PL_COM_1000_MDataSet(){}
	public PL_COM_1000_MDataSet(String errcode, String errmsg){
	...
	public void getValues(CallableStatement cstmt) throws SQLException{
		...
		while(rset0.next()){
			PL_COM_1000_MDLCO_CLSFCURRecord rec = new PL_COM_1000_MDLCO_CLSFCURRecord();
		...
		while(rset1.next()){
			PL_COM_1000_MAREACURRecord rec = new PL_COM_1000_MAREACURRecord();
		...
		while(rset2.next()){
			PL_COM_1000_MPRV_CURR_CLSFCURRecord rec = new PL_COM_1000_MPRV_CURR_CLSFCURRecord();
...
<%
	PL_COM_1000_MDataSet ds = (PL_COM_1000_MDataSet)request.getAttribute("ds");
%>
Web Tier에서 Record 객체 관련 코드 작성시 사용하십시오.

<%
	for(int i=0; i<ds.dlco_clsfcur.size(); i++){
		PL_COM_1000_MDLCO_CLSFCURRecord dlco_clsfcurRec = (PL_COM_1000_MDLCO_CLSFCURRecord)ds.dlco_clsfcur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.areacur.size(); i++){
		PL_COM_1000_MAREACURRecord areacurRec = (PL_COM_1000_MAREACURRecord)ds.areacur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.prv_curr_clsfcur.size(); i++){
		PL_COM_1000_MPRV_CURR_CLSFCURRecord prv_curr_clsfcurRec = (PL_COM_1000_MPRV_CURR_CLSFCURRecord)ds.prv_curr_clsfcur.get(i);%>
HTML 코드들....
	<%}%>
