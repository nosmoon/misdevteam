...
public class PL_TRN_1200_MDataSet extends somo.framework.db.BaseDataSet implements java.io.Serializable{
	...
	public PL_TRN_1200_MDataSet(){}
	public PL_TRN_1200_MDataSet(String errcode, String errmsg){
	...
	public void getValues(CallableStatement cstmt) throws SQLException{
		...
		while(rset5.next()){
			PL_TRN_1200_MMEDI_CDCURRecord rec = new PL_TRN_1200_MMEDI_CDCURRecord();
		...
		ResultSet rset6 = (ResultSet) cstmt.getObject(6);
		while(rset6.next()){
			PL_TRN_1200_MWH_CDCURRecord rec = new PL_TRN_1200_MWH_CDCURRecord();
		...
		ResultSet rset7 = (ResultSet) cstmt.getObject(7);
		while(rset7.next()){
			PL_TRN_1200_MDEAL_PTCRCURRecord rec = new PL_TRN_1200_MDEAL_PTCRCURRecord();
			...
			rec.prv_deli_qty = Util.checkString(rset7.getString("prv_deli_qty"));
			rec.prv_widr_qty = Util.checkString(rset7.getString("prv_widr_qty"));
			rec.prv_retn_qty = Util.checkString(rset7.getString("prv_retn_qty"));
...
<%
	PL_TRN_1200_MDataSet ds = (PL_TRN_1200_MDataSet)request.getAttribute("ds");
%>
Web Tier에서 Record 객체 관련 코드 작성시 사용하십시오.

<%
	for(int i=0; i<ds.medi_cdcur.size(); i++){
		PL_TRN_1200_MMEDI_CDCURRecord medi_cdcurRec = (PL_TRN_1200_MMEDI_CDCURRecord)ds.medi_cdcur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.wh_cdcur.size(); i++){
		PL_TRN_1200_MWH_CDCURRecord wh_cdcurRec = (PL_TRN_1200_MWH_CDCURRecord)ds.wh_cdcur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.deal_ptcrcur.size(); i++){
		PL_TRN_1200_MDEAL_PTCRCURRecord deal_ptcrcurRec = (PL_TRN_1200_MDEAL_PTCRCURRecord)ds.deal_ptcrcur.get(i);%>
HTML 코드들....
	<%}%>
