...
public class PR_PRTEXEC_2002_LDataSet extends somo.framework.db.BaseDataSet implements java.io.Serializable{
	...
	public PR_PRTEXEC_2002_LDataSet(){}
	public PR_PRTEXEC_2002_LDataSet(String errcode, String errmsg){
	...
	public void getValues(CallableStatement cstmt) throws SQLException{
		...
		while(rset0.next()){
			PR_PRTEXEC_2002_LCURLIST_XRecord rec = new PR_PRTEXEC_2002_LCURLIST_XRecord();
		...
		while(rset1.next()){
			PR_PRTEXEC_2002_LCURLIST_YRecord rec = new PR_PRTEXEC_2002_LCURLIST_YRecord();
		...
		while(rset2.next()){
			PR_PRTEXEC_2002_LCURLIST_DRecord rec = new PR_PRTEXEC_2002_LCURLIST_DRecord();
			rec.new_flag = Util.checkString(rset2.getString("new_flag"));
			...
			rec.issu_clsf = Util.checkString(rset2.getString("issu_clsf"));
...
<%
	for(int i=0; i<ds.curlist_x.size(); i++){
		PR_PRTEXEC_2002_LCURLIST_XRecord curlist_xRec = (PR_PRTEXEC_2002_LCURLIST_XRecord)ds.curlist_x.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.curlist_y.size(); i++){
		PR_PRTEXEC_2002_LCURLIST_YRecord curlist_yRec = (PR_PRTEXEC_2002_LCURLIST_YRecord)ds.curlist_y.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.curlist_d.size(); i++){
		PR_PRTEXEC_2002_LCURLIST_DRecord curlist_dRec = (PR_PRTEXEC_2002_LCURLIST_DRecord)ds.curlist_d.get(i);%>
HTML 코드들....
	<%}%>
