...
/**
 *  발송-추가발송 발송처리 WorkBean
 */
public class PSSend1601WB {

    /**
     * 추가발송 발송처리 관리화면을 초기화시킨다.
     * 조회된 request값을 set한다.
    ...
    public void selectCombolist(HttpServletRequest req, HttpServletResponse res) throws AppException {
        ...
        PSSend1601EJBHome home = (PSSend1601EJBHome) JNDIManager.getInstance().getHome("PSSend1601EJB");
        try {
            PSSend1601EJB ejb = home.create();
    ...
    public void selectList(HttpServletRequest req, HttpServletResponse res) throws AppException {
        PS_L_SEND_ADDM_PROCDataSet ds = null;
        //사용자 세션정보
        String cmpycd      = Util.getSessionParameterValue(req, "cmpycd", true);          // 회사코드
        ...
        PS_L_SEND_ADDM_PROCDM dm = new PS_L_SEND_ADDM_PROCDM();
        //dm에 값을 Setting한다.
        dm.setCmpycd(cmpycd);
        dm.setSenddt(senddt);
        dm.setBocd(bocd);
    ...
    public void selectMthdList(HttpServletRequest req, HttpServletResponse res) throws AppException {
        PS_L_SEND_ADDM_PROC_DTLSDataSet ds = null;
        ...
        PS_L_SEND_ADDM_PROC_DTLSDM dm = new PS_L_SEND_ADDM_PROC_DTLSDM();
        //dm에 값을 Setting한다.
        dm.setSenddt(senddt);
        dm.setBocd(bocd);
        dm.setMediser_no(mediser_no);
    ...
    public void printAddmSticker(HttpServletRequest req, HttpServletResponse res) throws AppException {
        PS_P_SEND_ADDM_STICKERDataSet ds = null;
        //사용자 세션정보
        String cmpycd      = Util.getSessionParameterValue(req, "cmpycd", true);          // 회사코드
        ...
        PS_P_SEND_ADDM_STICKERDM dm = new PS_P_SEND_ADDM_STICKERDM();
        //dm에 값을 Setting한다.
        dm.setCmpycd(cmpycd);
        dm.setSenddt(senddt);
        dm.setBocd(bocd);
        ...
        try {
            PSSend1601EJB ejb = home.create();
            ds = ejb.printAddmSticker(dm); // 설정된 dm값으로 EJB를 호출한다.
            req.setAttribute("ds", ds); // request에 결과값을 담는다.
        } catch (CreateException e) {
            SysException se = new SysException(e);
            LogManager.getInstance().log(se);
