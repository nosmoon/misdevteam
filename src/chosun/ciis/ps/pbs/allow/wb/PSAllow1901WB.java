...
public class PSAllow1901WB {
    ...
    public void selectRsltList(HttpServletRequest req, HttpServletResponse res) throws AppException {
        PS_L_ALON_RSLT_BASEDataSet ds = null;
        //로그인 사용자 세션정보
        String cmpycd =  Util.getSessionParameterValue(req, "cmpycd", true);      //출판회사코드
        String incmgpers =  Util.getSessionParameterValue(req, "uid", true);      //입력자아이디
        ...
        PS_L_ALON_RSLT_BASEDM dm = new PS_L_ALON_RSLT_BASEDM(); // DM 값을 Setting한다.
        dm.setCmpycd(cmpycd);
        dm.setBocd(bocd);
        dm.setAloncd(aloncd);
        ...
        PSAllow1901EJBHome home = (PSAllow1901EJBHome) JNDIManager.getInstance().getHome("PSAllow1901EJB");
        try {

            PSAllow1901EJB ejb = home.create();
