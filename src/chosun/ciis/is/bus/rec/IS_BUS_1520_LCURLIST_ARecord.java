/***************************************************************************************************
* 파일명 : .java
* 기능 : 독자우대-구독신청
* 작성일자 : 2007-05-22
* 작성자 : 김대섭
***************************************************************************************************/
/***************************************************************************************************
* 수정내역 :
* 수정자 :
* 수정일자 :
* 백업 : 
***************************************************************************************************/


package chosun.ciis.is.bus.rec;

import java.sql.*;
import chosun.ciis.is.bus.dm.*;
import chosun.ciis.is.bus.ds.*;

/**
 * 
 */


public class IS_BUS_1520_LCURLIST_ARecord extends java.lang.Object implements java.io.Serializable{

	public String dept_cd_nm;
	public String dm_tgt_amt;
	public String dm_rst_amt;
	public String dm_rate;
	public String jm_rst_amt;
	public String dm_j_rate;
	public String dt_tgt_amt;
	public String dy_tgt_amt;
	public String dy_rst_amt;
	public String dt_rate;
	public String dy_rate;
	public String jy_rst_amt;
	public String dy_j_rate;

	public IS_BUS_1520_LCURLIST_ARecord(){}

	public void setDept_cd_nm(String dept_cd_nm){
		this.dept_cd_nm = dept_cd_nm;
	}

	public void setDm_tgt_amt(String dm_tgt_amt){
		this.dm_tgt_amt = dm_tgt_amt;
	}

	public void setDm_rst_amt(String dm_rst_amt){
		this.dm_rst_amt = dm_rst_amt;
	}

	public void setDm_rate(String dm_rate){
		this.dm_rate = dm_rate;
	}

	public void setJm_rst_amt(String jm_rst_amt){
		this.jm_rst_amt = jm_rst_amt;
	}

	public void setDm_j_rate(String dm_j_rate){
		this.dm_j_rate = dm_j_rate;
	}

	public void setDt_tgt_amt(String dt_tgt_amt){
		this.dt_tgt_amt = dt_tgt_amt;
	}

	public void setDy_tgt_amt(String dy_tgt_amt){
		this.dy_tgt_amt = dy_tgt_amt;
	}

	public void setDy_rst_amt(String dy_rst_amt){
		this.dy_rst_amt = dy_rst_amt;
	}

	public void setDt_rate(String dt_rate){
		this.dt_rate = dt_rate;
	}

	public void setDy_rate(String dy_rate){
		this.dy_rate = dy_rate;
	}

	public void setJy_rst_amt(String jy_rst_amt){
		this.jy_rst_amt = jy_rst_amt;
	}

	public void setDy_j_rate(String dy_j_rate){
		this.dy_j_rate = dy_j_rate;
	}

	public String getDept_cd_nm(){
		return this.dept_cd_nm;
	}

	public String getDm_tgt_amt(){
		return this.dm_tgt_amt;
	}

	public String getDm_rst_amt(){
		return this.dm_rst_amt;
	}

	public String getDm_rate(){
		return this.dm_rate;
	}

	public String getJm_rst_amt(){
		return this.jm_rst_amt;
	}

	public String getDm_j_rate(){
		return this.dm_j_rate;
	}

	public String getDt_tgt_amt(){
		return this.dt_tgt_amt;
	}

	public String getDy_tgt_amt(){
		return this.dy_tgt_amt;
	}

	public String getDy_rst_amt(){
		return this.dy_rst_amt;
	}

	public String getDt_rate(){
		return this.dt_rate;
	}

	public String getDy_rate(){
		return this.dy_rate;
	}

	public String getJy_rst_amt(){
		return this.jy_rst_amt;
	}

	public String getDy_j_rate(){
		return this.dy_j_rate;
	}
}

/* 작성시간 : Tue Jun 19 09:32:29 KST 2012 */