...
public class SS_L_RCP_INITDataSet extends somo.framework.db.BaseDataSet implements java.io.Serializable{
	...
	public SS_L_RCP_INITDataSet(){}
	public SS_L_RCP_INITDataSet(String errcode, String errmsg){
	...
	public void getValues(CallableStatement cstmt) throws SQLException{
		...
		while(rset0.next()){
			SS_L_RCP_INITTEAMLISTRecord rec = new SS_L_RCP_INITTEAMLISTRecord();
		...
		while(rset1.next()){
			SS_L_RCP_INITPARTLISTRecord rec = new SS_L_RCP_INITPARTLISTRecord();
		...
		while(rset2.next()){
			SS_L_RCP_INITAREALISTRecord rec = new SS_L_RCP_INITAREALISTRecord();
		...
		while(rset3.next()){
			SS_L_RCP_INITCLAMTMTHDCURRecord rec = new SS_L_RCP_INITCLAMTMTHDCURRecord();
	...
	public String teamlistOptHtml(String selected){
		...
		for(int i=0; i<size; i++){
			SS_L_RCP_INITTEAMLISTRecord rec = (SS_L_RCP_INITTEAMLISTRecord)teamlist.get(i);
	...
	public String partlistOptHtml(String selected){
		...
		for(int i=0; i<size; i++){
			SS_L_RCP_INITPARTLISTRecord rec = (SS_L_RCP_INITPARTLISTRecord)partlist.get(i);
	...
	public String arealistOptHtml(String selected){
		...
		for(int i=0; i<size; i++){
			SS_L_RCP_INITAREALISTRecord rec = (SS_L_RCP_INITAREALISTRecord)arealist.get(i);
	...
	public String clamtmthdcurOptHtml(String selected){
		...
		for(int i=0; i<size; i++){
			SS_L_RCP_INITCLAMTMTHDCURRecord rec = (SS_L_RCP_INITCLAMTMTHDCURRecord)clamtmthdcur.get(i);
...
<%
	for(int i=0; i<ds.teamlist.size(); i++){
		SS_L_RCP_INITTEAMLISTRecord teamlistRec = (SS_L_RCP_INITTEAMLISTRecord)ds.teamlist.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.partlist.size(); i++){
		SS_L_RCP_INITPARTLISTRecord partlistRec = (SS_L_RCP_INITPARTLISTRecord)ds.partlist.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.arealist.size(); i++){
		SS_L_RCP_INITAREALISTRecord arealistRec = (SS_L_RCP_INITAREALISTRecord)ds.arealist.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.clamtmthdcur.size(); i++){
		SS_L_RCP_INITCLAMTMTHDCURRecord clamtmthdcurRec = (SS_L_RCP_INITCLAMTMTHDCURRecord)ds.clamtmthdcur.get(i);%>
HTML 코드들....
	<%}%>
...
/* 작성시간 : Fri Apr 10 13:44:20 KST 2009 */