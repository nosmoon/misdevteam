...
public class SL_A_COMM_APT_THRWDataSet extends somo.framework.db.BaseDataSet implements java.io.Serializable{
	...
	public SL_A_COMM_APT_THRWDataSet(){}
	public SL_A_COMM_APT_THRWDataSet(String errcode, String errmsg, String invsgno){
	...
	public void getValues(CallableStatement cstmt) throws SQLException{
		...
		this.invsgno = Util.checkString(cstmt.getString(36));
		ResultSet rset0 = (ResultSet) cstmt.getObject(37);
		while(rset0.next()){
			SL_A_COMM_APT_THRWMEDICURRecord rec = new SL_A_COMM_APT_THRWMEDICURRecord();
		...
		ResultSet rset1 = (ResultSet) cstmt.getObject(38);
		while(rset1.next()){
			SL_A_COMM_APT_THRWDLVSTATCURRecord rec = new SL_A_COMM_APT_THRWDLVSTATCURRecord();
	...
	public String medicurOptHtml(String selected){
		...
		for(int i=0; i<size; i++){
			SL_A_COMM_APT_THRWMEDICURRecord rec = (SL_A_COMM_APT_THRWMEDICURRecord)medicur.get(i);
	...
	public String dlvstatcurOptHtml(String selected){
		...
		for(int i=0; i<size; i++){
			SL_A_COMM_APT_THRWDLVSTATCURRecord rec = (SL_A_COMM_APT_THRWDLVSTATCURRecord)dlvstatcur.get(i);
...
<%
	SL_A_COMM_APT_THRWDataSet ds = (SL_A_COMM_APT_THRWDataSet)request.getAttribute("ds");
%>
Web Tier에서 Record 객체 관련 코드 작성시 사용하십시오.

<%
	for(int i=0; i<ds.medicur.size(); i++){
		SL_A_COMM_APT_THRWMEDICURRecord medicurRec = (SL_A_COMM_APT_THRWMEDICURRecord)ds.medicur.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.dlvstatcur.size(); i++){
		SL_A_COMM_APT_THRWDLVSTATCURRecord dlvstatcurRec = (SL_A_COMM_APT_THRWDLVSTATCURRecord)ds.dlvstatcur.get(i);%>
HTML 코드들....
	<%}%>
