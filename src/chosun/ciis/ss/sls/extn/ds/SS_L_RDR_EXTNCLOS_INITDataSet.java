...
public class SS_L_RDR_EXTNCLOS_INITDataSet extends somo.framework.db.BaseDataSet implements java.io.Serializable{
	...
	public SS_L_RDR_EXTNCLOS_INITDataSet(){}
	public SS_L_RDR_EXTNCLOS_INITDataSet(String errcode, String errmsg){
	...
	public void getValues(CallableStatement cstmt) throws SQLException{
		...
		while(rset0.next()){
			SS_L_RDR_EXTNCLOS_INITTEAMLISTRecord rec = new SS_L_RDR_EXTNCLOS_INITTEAMLISTRecord();
		...
		while(rset1.next()){
			SS_L_RDR_EXTNCLOS_INITPARTLISTRecord rec = new SS_L_RDR_EXTNCLOS_INITPARTLISTRecord();
		...
		while(rset2.next()){
			SS_L_RDR_EXTNCLOS_INITAREALISTRecord rec = new SS_L_RDR_EXTNCLOS_INITAREALISTRecord();
		...
		while(rset3.next()){
			SS_L_RDR_EXTNCLOS_INITCUREXTNTYPECDRecord rec = new SS_L_RDR_EXTNCLOS_INITCUREXTNTYPECDRecord();
		...
		while(rset4.next()){
			SS_L_RDR_EXTNCLOS_INITCURMEDICDRecord rec = new SS_L_RDR_EXTNCLOS_INITCURMEDICDRecord();
		...
		while(rset5.next()){
			SS_L_RDR_EXTNCLOS_INITCURSUBSCNFMSTATCDRecord rec = new SS_L_RDR_EXTNCLOS_INITCURSUBSCNFMSTATCDRecord();
		...
		while(rset6.next()){
			SS_L_RDR_EXTNCLOS_INITCURAPLCPATHCDRecord rec = new SS_L_RDR_EXTNCLOS_INITCURAPLCPATHCDRecord();
...
<%
	SS_L_RDR_EXTNCLOS_INITDataSet ds = (SS_L_RDR_EXTNCLOS_INITDataSet)request.getAttribute("ds");
%>
Web Tier에서 Record 객체 관련 코드 작성시 사용하십시오.

<%
	for(int i=0; i<ds.teamlist.size(); i++){
		SS_L_RDR_EXTNCLOS_INITTEAMLISTRecord teamlistRec = (SS_L_RDR_EXTNCLOS_INITTEAMLISTRecord)ds.teamlist.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.partlist.size(); i++){
		SS_L_RDR_EXTNCLOS_INITPARTLISTRecord partlistRec = (SS_L_RDR_EXTNCLOS_INITPARTLISTRecord)ds.partlist.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.arealist.size(); i++){
		SS_L_RDR_EXTNCLOS_INITAREALISTRecord arealistRec = (SS_L_RDR_EXTNCLOS_INITAREALISTRecord)ds.arealist.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.curextntypecd.size(); i++){
		SS_L_RDR_EXTNCLOS_INITCUREXTNTYPECDRecord curextntypecdRec = (SS_L_RDR_EXTNCLOS_INITCUREXTNTYPECDRecord)ds.curextntypecd.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.curmedicd.size(); i++){
		SS_L_RDR_EXTNCLOS_INITCURMEDICDRecord curmedicdRec = (SS_L_RDR_EXTNCLOS_INITCURMEDICDRecord)ds.curmedicd.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.cursubscnfmstatcd.size(); i++){
		SS_L_RDR_EXTNCLOS_INITCURSUBSCNFMSTATCDRecord cursubscnfmstatcdRec = (SS_L_RDR_EXTNCLOS_INITCURSUBSCNFMSTATCDRecord)ds.cursubscnfmstatcd.get(i);%>
HTML 코드들....
	<%}%>

<%
	for(int i=0; i<ds.curaplcpathcd.size(); i++){
		SS_L_RDR_EXTNCLOS_INITCURAPLCPATHCDRecord curaplcpathcdRec = (SS_L_RDR_EXTNCLOS_INITCURAPLCPATHCDRecord)ds.curaplcpathcd.get(i);%>
HTML 코드들....
	<%}%>
...
/* 작성시간 : Tue Dec 06 20:00:33 KST 2011 */