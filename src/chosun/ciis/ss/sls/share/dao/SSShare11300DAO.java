...
*/
public class SSShare11300DAO {

	/**
	* 정보공유 게시판 공지사항 초기 리스트 조회
	...
	* @param SS_MO_L_INFOEXG_SRCHDM 객체 NEW
	* @return SS_MO_L_INFOEXG_SRCHDataSet 객체 NEW
	* @throws AppException
	*/

	public SS_MO_L_INFOEXG_SRCHDataSet mo_selectSrchList(SS_MO_L_INFOEXG_SRCHDM dm) throws AppException { //모바일용 프로시저NEW
		DBManager manager = new DBManager("Oracle");
		SS_MO_L_INFOEXG_SRCHDataSet ds = (SS_MO_L_INFOEXG_SRCHDataSet)manager.executeCall(dm);
...
*/
	...
	/**
	 *  공지사항 상세정보 조회 // agency_commun_11120 참조
	 *
	 * @param SS_S_INFOEXGDM
	 * @return 통신실 게시판 공지사항 상세정보 DataSet
	 */

	public SL_S_INFOEXGDataSet mo_selectInfoexgDetail(SL_S_INFOEXGDM dm) //모바일용
			throws AppException {
		DBManager manager = new DBManager("Oracle");
		SL_S_INFOEXGDataSet ds = (SL_S_INFOEXGDataSet) manager.executeCall(dm);
/*		if (!"".equals(ds.errcode)) {
			throw new AppException(ds.errcode, ds.errmsg);
		}
*/
...
//               	String blobQuery = "SELECT FILECONT FROM TACOM_INFOEXG WHERE SEQ = '"+tempSeq+"' FOR UPDATE";
                   String blobQuery = "SELECT FILECONT FROM TACOM_INFOEXG WHERE SEQ = '"+tempSeq+"'";
...
//               	String blobQuery = "SELECT FILECONT FROM TACOM_INFOEXG WHERE SEQ = '"+tempSeq+"' FOR UPDATE";
	...
	* @param SS_S_INFOEXGDM
	* @return 정보공유 게시판 공지사항 상세정보 DataSet
	* @throws AppException
	*/
...
//            String blobQuery = "SELECT FILECONT FROM TACOM_INFOEXG WHERE SEQ = '"+dm.seq+"' FOR UPDATE";
            String blobQuery = "SELECT FILECONT FROM TACOM_INFOEXG WHERE SEQ = '"+dm.seq+"'";
            OracleResultSet rset = (OracleResultSet)stmt.executeQuery(blobQuery);
            if (rset.next()) {
                weblogic.jdbc.wrapper.Blob weblogic_blob = (weblogic.jdbc.wrapper.Blob)rset.getBlob(1);
                BLOB oracle_blob = (BLOB)weblogic_blob.getVendorObj();
            ...
            stmt.close();
            manager.disConnection();
/*20050708 김대섭 추가...20060608 이혁 제거
            ds.filenm = "/was/salesdept/applications/downloads/"+ds.filenm;
            FileOutputStream fos = new FileOutputStream(ds.filenm);
            fos.write(ds.filecont);
            fos.close();
20050708 김대섭 추가...20060608 이혁 제거 */
