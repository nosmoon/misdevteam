...
public class SSAdmin2100WB {

	//모바일관리-로그인승인 초기
    public void initMoAplc(HttpServletRequest req, HttpServletResponse res) throws AppException {
        ...
        SSAdmin2100DAO dao = new SSAdmin2100DAO();
        ds = dao.initMoAplc(dm);
        req.setAttribute("ds", ds);
    }


	//모바일관리-로그인승인 조회
    public void selectMoAplc(HttpServletRequest req, HttpServletResponse res) throws AppException {
        ...
        SSAdmin2100DAO dao = new SSAdmin2100DAO();
        ds = dao.selectMoAplc(dm);
        req.setAttribute("ds", ds);
    }


    //모바일관리-로그인이력리스트 2017-07-10 장선희 추가
    public void selectLoginHist(HttpServletRequest req, HttpServletResponse res) throws AppException {

    	// Request Parameter Processing
    	String boemp_id = Util.Uni2Ksc(Util.checkString(req.getParameter("boemp_id")));

    	// DM Setting
    	SS_MO_L_LOGINHISTDM dm = new SS_MO_L_LOGINHISTDM();

    	dm.setBoemp_id(boemp_id);
    	dm.print();

    	SS_MO_L_LOGINHISTDataSet ds = null;

    	SSAdmin2100DAO dao = new SSAdmin2100DAO();
    	ds = dao.selectLoginHist(dm);
    	req.setAttribute("ds", ds);
    }


    //모바일관리-기기변경,휴대번호변경이력리스트 2017-07-10 장선희 추가
    public void selectModiHist(HttpServletRequest req, HttpServletResponse res) throws AppException {

    	// Request Parameter Processing
    	String boemp_id = Util.checkString(req.getParameter("boemp_id"));

    	// DM Setting
    	SS_MO_L_MODIHISTDM dm = new SS_MO_L_MODIHISTDM();

    	dm.setBoemp_id(boemp_id);
    	dm.print();

    	SS_MO_L_MODIHISTDataSet ds = null;

    	SSAdmin2100DAO dao = new SSAdmin2100DAO();
    	ds = dao.selectModiHist(dm);
    	req.setAttribute("ds", ds);
    }


    //모바일관리-로그인승인 상세
    public void detailMoAplc(HttpServletRequest req, HttpServletResponse res) throws AppException {
        ...
        SSAdmin2100DAO dao = new SSAdmin2100DAO();
        ds = dao.detailMoAplc(dm);
        req.setAttribute("ds", ds);
    }

	//모바일관리-로그인승인 담당승인, 수정, 비밀번호 초기화
    public void updateMoAplc(HttpServletRequest req, HttpServletResponse res) throws AppException {
        ...
        System.out.println("errmsg="+errmsg);
         ...
         //담당승인할 때 보내기 승인, 거절, 초기화만. 차단, 차단해제은 보내지 않기.
        if (ds!=null && (workflag != "4" || workflag != "5")) {
        	System.out.println("SSAdmin2100WB.java");
...
//              jData.put("message", "EXTN"); test
                ...
                System.out.println("SSAdmin2100WB.java_APRV");
