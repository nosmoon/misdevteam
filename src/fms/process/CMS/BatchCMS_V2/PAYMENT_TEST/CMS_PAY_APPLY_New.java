...
*/
		...
		log.tgrLog("송신", BATCH_CMS_Sample_Telegrams_New.CMS_PAY_HEADER_New);
		// 여러 건 일때
		
		/*for(int i=0; i<20000 ;i++){
			conn.sendData(UtilMethod.loof(i));
			log.tgrLog("송신", UtilMethod.loof(i));
		}*/
		for(int i=0;i<BATCH_CMS_Sample_Telegrams_New.CMS_PAY_N_DATA_New.length;i++){
			...
			log.tgrLog("송신", BATCH_CMS_Sample_Telegrams_New.CMS_PAY_N_DATA_New[i]);
		}
		// 한 건 일때
		
//		conn.sendData(BATCH_CMS_Sample_Telegrams.CMS_ONE_MEM_N_DATA);
//		log.tgrLog("송신", BATCH_CMS_Sample_Telegrams.CMS_ONE_MEM_N_DATA);


		//  수정
//		conn.sendData(BATCH_CMS_Sample_Telegrams.CMS_MEM_U_DATA);
//		log.tgrLog("송신", BATCH_CMS_Sample_Telegrams.CMS_MEM_U_DATA);

		// 삭제
//		conn.sendData(BATCH_CMS_Sample_Telegrams.CMS_MEM_D_DATA);
//		log.tgrLog("송신", BATCH_CMS_Sample_Telegrams.CMS_MEM_D_DATA);
		
		conn.sendData(BATCH_CMS_Sample_Telegrams_New.CMS_PAY_TAIL_New);
		log.tgrLog("송신", BATCH_CMS_Sample_Telegrams_New.CMS_PAY_TAIL_New);
