 ...
 *
 */
public class ResinJNDIManager{
  /**
   * Singleton 패턴 적용 멤버변수
   */
  private static ResinJNDIManager m_instance;
  /**
  ...
  private ResinJNDIManager(){
    ...
    try{
      ...
      FWLogManager.getInstance().log("ResinJNDIManager.java", "InitialContext()", e,
   ...
   */
  public static ResinJNDIManager getInstance(){
    if(m_instance == null){
      m_instance = new ResinJNDIManager();
    }
    return m_instance;
  }
  ...
  public synchronized Object getHome(String jndiName){
    ...
    try{
      ...
      FWLogManager.getInstance().log("ResinJNDIManager.java","getHome()", e, "ResinJNDIManager를 이용하여 EJBHome 객체를 얻는 과정에서 예외가 발생했습니다.", "JNDI 관련부분을 확인하십시오.", false);
      throw se;
    }
  ...
  public synchronized DataSource getDataSource(String jndiName){
    ...
    try{
      ...
      SysException se = new SysException(e);
      FWLogManager.getInstance().log("ResinJNDIManager.java","getDataSource()", e, "ResinJNDIManager를 이용하여 DataSource 객체를 얻는 과정에서 예외가 발생했습니다.", "JNDI 관련부분을 확인하십시오.", false);
      throw se;
    }
    return obj;
  }
